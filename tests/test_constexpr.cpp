...
// Joins
static_assert(
    np::concatenate(np::ndarrayf<int, 3>{0, 1, 2}, np::ndarrayf<int, 2>{0, 1}).size_v
    == 5);
...
static_assert(
    np::linalg::slogdet(SQ).sign == 1.0
    && close(np::linalg::slogdet(SQ).logabsdet, 2.0794415416798357));
static_assert(
    np::linalg::solve(SQ, np::ndarrayf<int, 2>{2, 4})[0] == 1.0
...
static_assert(
    np::linalg::norm(np::ndarrayf<double, 4>{1, 2, 3, 4}, np::linalg::NormOrd::One)
    == 10.0);
static_assert(
    np::linalg::norm(np::ndarrayf<double, 4>{1, 2, 3, 4}, np::linalg::NormOrd::Inf)
    == 4.0);
static_assert(close(
    np::linalg::norm(np::ndarrayf<double, 4>{1, 2, 3, 4}, np::linalg::NormOrd::NegOne),
    0.48));
static_assert(
    close(np::linalg::norm(np::ndarrayf<double, 2, 2>{{1, 0}, {0, 2}}), std::sqrt(5.0)));
static_assert(
    np::linalg::norm(np::ndarrayf<double, 2, 2>{{1, 0}, {0, 2}}, np::linalg::NormOrd::Nuc)
    == 3.0);
...
constexpr auto EH =
    np::linalg::eigh(np::ndarrayf<double, 3, 3>{{2, 0, 0}, {0, 3, 0}, {0, 0, 5}});
static_assert(EH.w[0] == 2.0 && EH.w[1] == 3.0 && EH.w[2] == 5.0);
static_assert(np::linalg::eigvalsh(np::ndarrayf<double, 2, 2>{{1, 3}, {3, 4}})[1] > 5.0);

static_assert(
    np::linalg::cross(
        np::ndarrayf<double, 3>{1, 0, 0}, np::ndarrayf<double, 3>{0, 1, 0})[2]
    == 1.0);
static_assert(
    np::linalg::outer(np::ndarrayf<double, 2>{1, 2}, np::ndarrayf<double, 3>{3, 4, 5})(
        1, 2)
    == 10.0);
static_assert(
    np::linalg::inner(np::ndarrayf<double, 3>{1, 2, 3}, np::ndarrayf<double, 3>{4, 5, 6})
    == 32.0);
constexpr auto LS = np::linalg::lstsq(
    np::ndarrayf<double, 3, 2>{{1, 0}, {0, 1}, {0, 0}}, np::ndarrayf<double, 3>{1, 2, 0});
