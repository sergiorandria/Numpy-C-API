...
int main() {
    ...
    // choose (default 'r', wrap, clip, broadcasting)
    {
        ...
        test::check(out.size() == 3 && out[0] == 40 && out[1] == 20 && out[2] == 90,
        ...
        test::check(throws_any([&] {
                        cl.choose(std::vector<Ndarray<int>>{w0, w1});
                    }),
                    "choose raise mode throws on out-of-range");

        Ndarray<int> sc = {99};
        auto bc = idx.choose(std::vector<Ndarray<int>>{sc}, 'c');
        test::check(bc[0] == 99 && bc[1] == 99 && bc[2] == 99,
                    "choose broadcasts choices");
    }
    ...
    // compress (axis and flattened)
    {
        ...
        test::check(c.ndim() == 2 && c.shape[0] == 2 && c.shape[1] == 2,
                    "compress axis 1 shape");
        test::check(c(0, 0) == 1 && c(0, 1) == 3 && c(1, 0) == 4 && c(1, 1) == 6,
                    "compress axis 1 values");

        Ndarray<bool> cond0 = {true, false};
        auto c0 = arr.compress(cond0, 0);
        test::check(c0.shape[0] == 1 && c0.shape[1] == 3 && c0(0, 2) == 3,
                    "compress axis 0");

        Ndarray<bool> flatc = {true, false, true, true, true, false};
        auto flat = arr.compress(flatc);
        test::check(flat.ndim() == 1 && flat.size() == 4,
                    "compress flattened shape");
        test::check(flat[0] == 1 && flat[1] == 3 && flat[2] == 4 && flat[3] == 5,
                    "compress flattened values");

        test::check(throws_arg([&] {
                        arr.compress(Ndarray<bool>{{true, false}}, 1);
                    }),
                    "compress wrong condition length throws");
        test::check(throws_arg([&] {
                        arr.compress(Ndarray<bool>{{true, false}, {true, true}});
                    }),
    ...
    // dot / matmul methods delegate to np::linalg
    {
        ...
        auto d = a.dot(b);
        test::check(d.ndim() == 2 && d.shape[0] == 2 && d.shape[1] == 2,
                    "method dot shape");
        test::check(d(0, 0) == 58 && d(0, 1) == 64 && d(1, 0) == 139 && d(1, 1) == 154,
                    "method dot values");
        auto m = a.matmul(b);
        test::check(m(1, 1) == 154, "method matmul values");
        Ndarray<int> v = {1, 2, 3};
        auto vd = a.dot(v);
        test::check(vd.ndim() == 1 && vd.size() == 2 && vd[0] == 14 && vd[1] == 32,
    ...
    // partition
    {
        ...
        test::check(a[0] <= 3 && a[1] <= 3 && a[3] >= 3 && a[4] >= 3,
    ...
    // real / imag
    {
        ...
        auto re = z.real();
        auto im = z.imag();
        test::check(re.size() == 2 && test::approx(re[0], 1.0) && test::approx(re[1], -3.0),
                    "real of complex");
        test::check(im.size() == 2 && test::approx(im[0], 2.0) && test::approx(im[1], 4.0),
                    "imag of complex");
        ...
        auto ii = r.imag();
        test::check(ii.size() == 3 && ii[0] == 0 && ii[1] == 0 && ii[2] == 0,
    ...
    // mT
    {
        ...
        auto m3 = t.mT();
        test::check(m3.shape[0] == 2 && m3.shape[1] == 4 && m3.shape[2] == 3,
                    "mT 3D shape");
        test::check(m3.get(std::array<std::size_t, 3>{1, 2, 0}) == 14,
                    "mT 3D swaps last two axes");

        test::check(throws_any([&] { Ndarray<int>{1, 2}.mT(); }),
                    "mT ndim < 2 throws");
    }
    ...
    {
        Ndarray<int> a = {1, 2};
        test::check(a.owns_data() && !a.is_view() && a.base() == nullptr,
                    "owned array attributes");
        test::check(a.writeable(), "default writeable");
        a.setflags(false);
        test::check(!a.writeable(), "setflags(false)");

        auto v = a.view();
        test::check(v.is_view() && !v.owns_data() && v.base() != nullptr,
                    "view borrows storage");
        auto c = a.copy();
        test::check(!c.is_view() && c.owns_data(), "copy owns its storage");

        Ndarray<int> m = {{1, 2, 3}, {4, 5, 6}};
        test::check(m.is_contiguous() && !m.is_f_contiguous(),
                    "C-order is not F-contiguous");
        auto mt = m.transpose();
        test::check(!mt.is_contiguous() && mt.is_f_contiguous(),
                    "transposed is F-contiguous");
    }
    ...
    // flat / len / contains
    {
        ...
        test::check(throws_arg([&] {
                        (void)Ndarray<int>(std::vector<int>{}).len();
                    }),
                    "len of 0-d throws");
    }
    ...
    // floordiv / divmod / pow
    {
        ...
        auto dm = x.divmod(3);
        test::check(dm.first[0] == 2 && dm.first[1] == -3 && dm.first[2] == 3,
                    "divmod quotient");
        test::check(dm.second[0] == 1 && dm.second[1] == 1 && dm.second[2] == 0,
                    "divmod remainder");
    ...
    // operators: %, &, |, ^, ~, <<, >>, unary +/-
    {
        ...
        auto nb = ~u;
        test::check(nb[0] == static_cast<unsigned>(~5u) &&
                        nb[1] == static_cast<unsigned>(~3u),
                    "bitwise not");
    ...
    // scalar conversions
    {
        ...
        test::check(test::approx_c(static_cast<std::complex<double>>(z),
                                   std::complex<double>(1.0, 2.0)),
                    "complex conversion");

        test::check(throws_arg([&] {
                        (void)static_cast<double>(Ndarray<int>{1, 2});
                    }),
