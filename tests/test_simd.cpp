...
void test_sum_f32() {
    ...
    test::check(test::approx(result, expected, 1e-3f),
                "sum_vectorized f32");
}
...
void test_sum_f64() {
    ...
    test::check(test::approx(result, expected, 1e-8),
                "sum_vectorized f64");
}
...
void test_large_arrays() {
    ...
    test::check(!std::isnan(sum) && !std::isinf(sum),
                "large array sum is finite");
}
