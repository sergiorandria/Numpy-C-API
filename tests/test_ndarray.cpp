...
int main()
{
  ...
  // Nested initializer-list construction
  {
    ...
    test::check(
        a.ndim() == 2 && a.shape[0] == 2 && a.shape[1] == 3, "nested init list shape");
  ...
  // Sorting
  {
    ...
    test::check(
        sorted.searchsorted(np::ndarray<int>{0, 4, 9})[1] == 2, "searchsorted vector");
  }
  ...
  // diagonal / trace / nonzero
  {
    ...
    auto nz = z.nonzero();
    test::check(
        nz.size() == 2 && nz[0](0) == 0 && nz[1](0) == 1 && nz[0](1) == 1
            && nz[1](1) == 1,
        "nonzero");
  }
