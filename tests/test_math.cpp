...
int main() {
    ...
    // --- New ufuncs: copysign, logaddexp, logaddexp2 ---
    {
        ...
        test::check(test::approx(le.at(2), std::log1p(1.0), 1e-6),
                    "logaddexp(1e-300, 1e-300) = ln(2) ~ 0.693");
        test::check(test::approx(le.at(3), 1e300 + std::log1p(0.0), 1e-6),
        ...
        test::check(test::approx(le2.at(3), a.at(3), 1e-9),
                    "logaddexp2(1e300, 1e-300) ~ 1e300");
    }
    ...
    // --- sign(NaN) propagates; complex sign unit vector ---
    {
        ...
        auto sc = sign(c);
        test::check(test::approx_c(sc.at(0), std::complex<double>{0.6, 0.8}),
                    "sign(3+4i) = (3+4i)/5");
    }
    ...
    // --- fma single-pass correctness ---
    {
        ...
        auto res = fma(a, b, c);
        test::check(res.at(0) == std::fma(A, B, C),
                    "fma(A,B,C) == std::fma (single rounding step)");
        test::check(res.at(0) != A * B + C,
                    "fma(A,B,C) != A*B+C (two-pass loses 2^-54)");
    }
    ...
    // --- out= overloads (unary, binary, ternary, aliases) ---
    {
        ...
        maximum(mx, mn, mout);
        test::check(mout.at(0) == 4.0 && mout.at(1) == 5.0 && mout.at(2) == 6.0,
                    "maximum out= value");
        ...
        test::check(bout.at(0, 0) == 4.0 && bout.at(0, 1) == 5.0 &&
                        bout.at(1, 0) == 4.0 && bout.at(1, 1) == 7.0,
        ...
        mod(mx, mn, dout);
        test::check(test::approx(dout.at(0), std::remainder(1.0, 4.0)),
        ...
        // nan_to_num with out=
        auto nanx = asarray(std::vector<double>{std::numeric_limits<double>::quiet_NaN(),
                                                std::numeric_limits<double>::infinity()});
