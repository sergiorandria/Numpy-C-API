...
int main() {
  ...
  // polyval
  {
    ...
    auto y = polyval(p, x);
    check(approx(y.at(0),2.0) && approx(y.at(1),0.0) && approx(y.at(2),0.0) && approx(y.at(3),2.0), "polyval array");
  ...
  // roots – linear, quadratic, cubic, quartic via companion eig
  {
    ...
    for(size_t i=0;i<r2.size();++i){ if(approx_c(r2.at(i),{1,0})) has1=true; if(approx_c(r2.at(i),{2,0})) has2=true; }
    ...
    for(size_t i=0;i<3;++i){ if(approx(r3.at(i).real(),1.0,1e-6)) c1=true; if(approx(r3.at(i).real(),2.0,1e-6)) c2=true; if(approx(r3.at(i).real(),3.0,1e-6)) c3=true; }
    ...
    for(size_t i=0;i<4;++i){ double re=r4.at(i).real(); if(approx(re,1.0,1e-5)) q1=true; if(approx(re,2.0,1e-5)) q2=true; if(approx(re,3.0,1e-5)) q3=true; if(approx(re,4.0,1e-5)) q4=true; }
  ...
  if (test::failures()==0) { std::printf("OK polynomial\n"); return 0; }
  return 1;
}
