...
static np::linalg::EigenResult<double> check_eig(const ndarray<int>& a)
{
  ...
  for (int j = 0; j < n; ++j)
  {
    for (int i = 0; i < n; ++i)
    {
      ...
      test::check(
          std::abs(lhs - rhs) < 1e-7 * (std::abs(lhs) + std::abs(rhs) + 1.0),
...
int main()
{
  ...
  // 2x3, full matrices: shapes, ordering, orthonormality, reconstruction
  {
    ...
    test::check(
        test::approx(r.s(0) * r.s(0) + r.s(1) * r.s(1), 91.0, 1e-12),
        "svd 2x3 frobenius invariant");
    test::check(
        test::approx(r.s(0) * r.s(1), std::sqrt(54.0), 1e-12),
        "svd 2x3 product invariant");
    auto utu = linalg::dot(r.u.transpose(), r.u);
    test::check(
        test::approx(utu(0, 0), 1.0) && test::approx(utu(0, 1), 0.0)
            && test::approx(utu(1, 1), 1.0),
        "svd u orthonormal");
    auto vvt = linalg::dot(r.vh, r.vh.transpose());
    test::check(
        test::approx(vvt(0, 0), 1.0) && test::approx(vvt(0, 1), 0.0)
    ...
    for (int i = 0; i < 2; ++i)
    {
      ...
      {
        test::check(
            test::approx(rec(i, j), static_cast<double>(a(i, j)), 1e-12),
  ...
  // rank-deficient: A = [[1,2],[2,4]] has sigma = {5, 0}
  {
    ...
    auto rec = linalg::dot(
        linalg::dot(r.u, ndarray<double>{{r.s(0), 0.0}, {0.0, r.s(1)}}), r.vh);
    for (int i = 0; i < 2; ++i)
    {
      for (int j = 0; j < 2; ++j)
      {
  ...
  // 3x2 (m > n) and the m < n transpose path
  {
    ...
    test::check(
        test::approx(full.s(0), red.s(0)) && test::approx(full.s(1), red.s(1)),
  ...
  // reduced mode on 3x2: reconstruction, orthonormality, triangularity
  {
    ...
    test::check(
        test::approx(qtq(0, 0), 1.0) && test::approx(qtq(0, 1), 0.0)
  ...
  // complete mode: q (M, M), r (M, N)
  {
    ...
    test::check(
        test::approx(r.r(2, 0), 0.0) && test::approx(r.r(2, 1), 0.0),
  ...
  // r mode and raw mode
  {
    ...
    for (int i = 0; i < 2; ++i)
    {
      ...
      {
        test::check(
            test::approx(raw.h(i, j), r.r(i, j), 1e-14),
    ...
    for (int i = 0; i < 3; ++i)
    {
      for (int j = 0; j < 2; ++j)
      {
        ...
        test::check(
            test::approx(rec, static_cast<double>(a(i, j)), 1e-12),
  ...
  // complex pair from a rotation block
  {
    ...
    test::check(
        test::approx(sum.real(), 3.0) && test::approx(sum.imag(), 0.0),
  ...
  // same); here we only verify the double eigenvalue.
  {
    ...
    auto e = linalg::eig(a);
    test::check(
        test::approx(e.w(0).real(), 1.0) && test::approx(e.w(0).imag(), 0.0)
  ...
  // 4x4 block diagonal with two complex pairs: trace and det invariants
  {
    ...
    test::check(
        test::approx(sum.real(), 0.0) && test::approx(sum.imag(), 0.0),
        "eig 4x4 trace invariant");
    test::check(
        test::approx(prod.real(), 4.0) && test::approx(prod.imag(), 0.0),
  ...
  // eigvals and the 1x1 / empty / triangular edge cases
  {
    ...
    test::check(
        test::approx(w(0).real(), 3.0) && test::approx(w(1).real(), 3.0),
        "eigvals values");
  }
  {
    ...
    auto e = linalg::eig(a);
    test::check(
        test::approx(e.w(0).real(), 7.0) && test::approx(e.w(0).imag(), 0.0)
  ...
  }
  {
    ...
    auto e = check_eig(a);
    test::check(
        test::approx(e.w(0).real(), 1.0) && test::approx(e.w(1).real(), 4.0)
...

  {
    ...
    test::check(
        test::approx(sl.sign, -1.0) && test::approx(sl.logabsdet, std::log(2.0)),
        "slogdet 2x2");
  }
  {
    ...
    auto sl = linalg::slogdet(a);
    test::check(
        sl.sign == 0.0 && std::isinf(sl.logabsdet) && sl.logabsdet < 0.0,
        "slogdet singular");
  }
  {
    ndarray<double> a(std::vector<int>{0, 0});
    ...
    auto sl = linalg::slogdet(a);
    test::check(
        test::approx(sl.sign, 1.0) && test::approx(sl.logabsdet, 0.0), "slogdet 0x0");
  }
  {
    ndarray<int> a = ndarray<int>::from_data(std::vector<int>{1, 1}, std::vector<int>{7});
...

  {
    ...
    auto i = linalg::inv(a);
    test::check(
        test::approx(i(0, 0), 0.6) && test::approx(i(0, 1), -0.7)
            && test::approx(i(1, 0), -0.2) && test::approx(i(1, 1), 0.4),
        "inv 2x2 values");
    auto id = linalg::matmul(a, i);
    test::check(
        test::approx(id(0, 0), 1.0) && test::approx(id(0, 1), 0.0)
...

  {
    ...
    test::check(
        x.ndim() == 1 && test::approx(x(0), 2.0) && test::approx(x(1), 3.0),
        "solve 1D rhs");
    ndarray<int> b2{{9, 9}, {8, 8}};
    auto x2 = linalg::solve(a, b2);
    test::check(
        x2.ndim() == 2 && test::approx(x2(0, 0), 2.0) && test::approx(x2(0, 1), 2.0)
            && test::approx(x2(1, 0), 3.0) && test::approx(x2(1, 1), 3.0),
        "solve 2D rhs");
    auto chk = linalg::matmul(a, x2);
    test::check(
        test::approx(chk(0, 1), 9.0) && test::approx(chk(1, 0), 8.0), "solve round trip");
  }
  {
    ndarray<int> a{{1, 2}, {2, 4}};
    ndarray<int> b{1, 1};
...

  {
    ...
    test::check(
        test::approx(p(0, 0), 89.0) && test::approx(p(1, 0), 55.0),
        "matrix_power fibonacci");
    auto z = linalg::matrix_power(a, 0);
    test::check(
        test::approx(z(0, 0), 1.0) && test::approx(z(1, 0), 0.0), "matrix_power zero");
    auto m = linalg::matrix_power(a, -1);
    test::check(
        test::approx(m(0, 0), 0.0) && test::approx(m(0, 1), 1.0)
...

  {
    ...
    test::check(
        test::approx(l(0, 0), 2.0) && test::approx(l(1, 0), 1.0)
            && test::approx(l(0, 1), 0.0) && test::approx(l(1, 1), std::sqrt(2.0)),
        "cholesky lower");
    auto rec = linalg::matmul(l, l.transpose());
    test::check(
        test::approx(rec(1, 0), 2.0) && test::approx(rec(1, 1), 3.0),
        "cholesky lower round trip");
    auto u = linalg::cholesky(a, true);
    test::check(
        test::approx(u(0, 0), 2.0) && test::approx(u(0, 1), 1.0)
...

  {
    ...
    test::check(
        test::approx(linalg::norm(v, linalg::NormOrd::One), 7.0), "norm vector 1");
    test::check(
        test::approx(linalg::norm(v, linalg::NormOrd::Inf), 4.0), "norm vector inf");
    test::check(
        test::approx(linalg::norm(v, linalg::NormOrd::NegInf), 3.0), "norm vector -inf");
    test::check(
        test::approx(linalg::norm(v, linalg::NormOrd::NegOne), 12.0 / 7.0),
        "norm vector -1");
  }
  {
    ndarray<int> m{{1, 2}, {3, 4}};
    ...
    test::check(
        test::approx(linalg::norm(m, linalg::NormOrd::One), 6.0), "norm matrix 1");
    test::check(
        test::approx(linalg::norm(m, linalg::NormOrd::Inf), 7.0), "norm matrix inf");
    test::check(
        test::approx(linalg::norm(m, linalg::NormOrd::NegOne), 4.0), "norm matrix -1");
    test::check(
        test::approx(linalg::norm(m, linalg::NormOrd::NegInf), 3.0), "norm matrix -inf");
    ...
    test::check(
        test::approx(linalg::norm(m, linalg::NormOrd::NegTwo), s1), "norm matrix -2");
  }
...

  {
    ...
    test::check(
        test::approx(e.w(0), 1.0) && test::approx(e.w(1), 3.0), "eigh values ascending");
    for (int j = 0; j < 2; ++j)
    {
    ...
    test::check(
        test::approx(vtv(0, 0), 1.0) && test::approx(vtv(0, 1), 0.0)
...

  {
    ...
    test::check(
        t0.ndim() == 4 && t0.get(std::vector<std::size_t>{0, 0, 0, 0}) == 5
...

  {
    ...
    test::check(
        cm.ndim() == 2 && cm(0, 0) == 0 && cm(1, 0) == 0 && cm(2, 0) == 0,
        "cross axis 0 self");
    // 1-D x 2-D broadcast
...

  {
    ...
    test::check(
        md.shape[0] == 2 && md(0, 0) == 5 && md(1, 1) == 9, "multi_dot 3 matrices");
    ...
    test::check(
        mo.shape[0] == 10 && mo.shape[1] == 50 && mo(0, 0) == 1,
  ...
  }
  {
    ...
    test::check(
        test::approx(chk(0), 1.0) && test::approx(chk(1), 2.0)
...

  {
    ...
    test::check(
        ds.shape[0] == 2 && ds.shape[1] == 2 && ds(0, 0) == 0 && ds(0, 1) == 3
            && ds(1, 0) == 4 && ds(1, 1) == 7,
        "diagonal 3D stack");
  }
...

  {
    ...
    test::check(
        t.shape[0] == 3 && t.shape[1] == 2 && t(0, 1) == 4 && t(2, 0) == 3,
        "matrix_transpose 2D");
    ndarray<int> s(std::vector<int>{2, 2, 2});
    for (std::size_t i = 0; i < 8; ++i)
    {
...

  {
    ndarray<int> m{{1, 2}, {3, 4}};
    test::check(
        test::approx(linalg::matrix_norm(m), 5.477225575051661),
        "matrix_norm default fro");
    test::check(
        test::approx(
            linalg::matrix_norm(m, linalg::NormOrd::Fro),
            linalg::norm(m, linalg::NormOrd::Fro)),
        "matrix_norm fro == norm fro");
    test::check(
        test::approx(linalg::matrix_norm(m, linalg::NormOrd::Nuc), 5.830951894845301),
        "matrix_norm nuc");
    test::check(
        test::approx(
            linalg::matrix_norm(m, linalg::NormOrd::Inf),
            linalg::norm(m, linalg::NormOrd::Inf)),
        "matrix_norm inf");
    test::check(
        test::approx(
            linalg::matrix_norm(m, linalg::NormOrd::Two),
...

  {
    ...
    test::check(
        ai.shape[0] == 8 && ai.shape[1] == 3 && ai.shape[2] == 4 && ai.shape[3] == 6,
    ...
    test::check(
        ai1.shape[0] == 8 && ai1.shape[1] == 3 && ai1.shape[2] == 24,
...

  {
    ...
    test::check(
        x.shape[0] == 2 && x.shape[1] == 3 && x.shape[2] == 4
    ...
    test::check(
        x2.shape[0] == 4 && x2.shape[1] == 6 && x2(0, 1) == 1 && x2(1, 2) == 8
            && x2(2, 0) == 12 && x2(3, 3) == 21,
...

  {
    ...
    test::check(
        p.ndim() == 1 && test::approx(p(0), 3.0) && test::approx(p(1), 8.0)
            && test::approx(p(2), 10.0),
        "vecdot doc example");
    ...
    test::check(
        c0.ndim() == 1 && c0(0) == 5 && c0(1) == 7 && c0(2) == 9, "vecdot axis 0");
    ...
    test::check(
        bc.shape[0] == 2 && bc.shape[1] == 3 && bc(0, 0) == 6 && bc(1, 2) == 86,
        "vecdot broadcast");
    bool threw = false;
    try
    {
...

  {
    ...
    test::check(
        all.ndim() == 0 && test::approx(all.item(), 16.881943016134134),
        "vector_norm default");
    test::check(
        test::approx(linalg::vector_norm(b, {}, false, 0.0).item(), 9.0),
        "vector_norm ord 0");
    test::check(
        test::approx(linalg::vector_norm(b, {}, false, 1.0).item(), 45.0),
        "vector_norm ord 1");
    test::check(
        test::approx(linalg::vector_norm(b, {}, false, -1.0).item(), 0.3534857623790153),
        "vector_norm ord -1");
    test::check(
        test::approx(linalg::vector_norm(b, {}, false, 2.0).item(), 16.881943016134134),
        "vector_norm ord 2");
    test::check(
        test::approx(linalg::vector_norm(b, {}, false, -2.0).item(), 0.8058837395885292),
        "vector_norm ord -2");
    test::check(
        test::approx(
            linalg::vector_norm(b, {}, false, std::numeric_limits<double>::infinity())
                .item(),
            9.0),
        "vector_norm ord inf");
    test::check(
    ...
    test::check(
        both.ndim() == 0 && test::approx(both.item(), 16.881943016134134),
        "vector_norm two axes");
    auto kd = linalg::vector_norm(b, std::vector<int>{0}, true);
    test::check(
        kd.shape[0] == 1 && kd.shape[1] == 3 && test::approx(kd(0, 0), 8.12403840463596),
        "vector_norm keepdims");
    auto kdall = linalg::vector_norm(b, {}, true);
    test::check(
        kdall.shape[0] == 1 && kdall.shape[1] == 1
