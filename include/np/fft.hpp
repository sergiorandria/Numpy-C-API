...
namespace np::fft {
    ...
    namespace detail {
        ...
        inline Cplx twiddle(int n, int k, bool inverse) {
            const double angle =
                (inverse ? 2.0 : -2.0) * std::numbers::pi_v<double> *
            ...
            for (std::size_t len = 2; len <= n; len <<= 1) {
                ...
                for (std::size_t i = 0; i < n; i += len) {
                    for (std::size_t k = 0; k < half; ++k) {
                        const Cplx w = twiddle(static_cast<int>(len),
                                               static_cast<int>(k), inverse);
                        const Cplx u = a[i + k];
     ...
     */
    template <typename T>
    auto fft(const Ndarray<T>& x) -> Ndarray<Cplx> {
        ...
        Ndarray<Cplx> out =
            Ndarray<Cplx>::from_data(x.shape,
                                     std::vector<Cplx>(x._numel(), Cplx{0.0, 0.0}));
     ...
     */
    template <typename T>
    auto ifft(const Ndarray<T>& x) -> Ndarray<Cplx> {
        const std::size_t nd = x.ndim();
        if (nd == 0) {
    ...
    template <typename T>
    auto abs(const Ndarray<T>& x) -> Ndarray<double> {
