...
template <typename LA, typename LB> struct merged {
  ...
  using PA =
      typename pad<A, static_cast<int>(ra), static_cast<int>(rank)>::type;
  using PB =
      typename pad<B, static_cast<int>(rb), static_cast<int>(rank)>::type;
...
template <int Head, int... Tail> struct squeeze_tag<shape_tag<Head, Tail...>> {
  ...
  using type =
      std::conditional_t<Head == 1, rest, typename prepend<Head, rest>::type>;
};
