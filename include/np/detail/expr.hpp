...
namespace np::detail::expr {
    ...
    struct shape_tag_of<ndarray<T, E...>> {
        using type = shape_tag<E...>;
    };

    template <typename S>
    using shape_tag_t =
        typename shape_tag_of<std::remove_cv_t<std::remove_reference_t<S>>>::type;
    ...
    template <int Head, typename Tag>
    struct prepend;

    template <int Head, int... E>
    struct prepend<Head, shape_tag<E...>> {
     ...
     */
    template <typename Tag, int Cur, int Target>
    struct pad;

    template <int... E, int Cur, int Target>
    struct pad<shape_tag<E...>, Cur, Target>
        : pad<shape_tag<1, E...>, Cur + 1, Target> {};

    template <int... E, int Target>
    struct pad<shape_tag<E...>, Target, Target> {
        using type = shape_tag<E...>;
    };
    ...
    template <int... A, int... B>
    struct zip_merge<shape_tag<A...>, shape_tag<B...>> {
    ...
    struct merged {
        ...
        using PA = typename pad<A, static_cast<int>(ra),
                                static_cast<int>(rank)>::type;
        using PB = typename pad<B, static_cast<int>(rb),
                                static_cast<int>(rank)>::type;
    ...
    inline constexpr bool broadcast_valid = merged<L, R>::valid;
    ...
    constexpr std::array<std::size_t, R> unflatten(
        std::size_t flat, const std::array<int, R>& shape) {
    ...
    constexpr typename S::value_type gather(
        const S& src, const std::array<std::size_t, R>& coords) {
    ...
    template <typename T>
    struct shape_tag_of<scalar_expr<T>> {
    ...
    class binary_expr {
        static_assert(merged<L, R>::valid,
                      "np: incompatible shapes for elementwise operation "
    ...
    public:
        using lhs_type = L;
        using rhs_type = R;
        using value_type = std::invoke_result_t<Op, typename L::value_type,
        ...
        constexpr auto eval() const { return eval_impl(tag{}); }

        using eval_type = decltype(std::declval<binary_expr>().eval_impl(tag{}));

        constexpr operator eval_type() const { return eval(); }
    };
    ...
    template <typename Op, typename L, typename R>
    struct shape_tag_of<binary_expr<Op, L, R>> {
        using type = typename binary_expr<Op, L, R>::tag;
    };

    template <typename Op, typename S>
    struct shape_tag_of<unary_expr<Op, S>> {
    ...
    template <int... Acc>
    struct rev_impl<shape_tag<Acc...>, shape_tag<>> {
    ...
    struct reverse : rev_impl<shape_tag<>, Tag> {};

    /** @brief Remove the extent at zero-based position `Axis`. */
    template <int Axis, int Cur, typename Tag>
    struct remove_at;

    template <int Axis, int Cur>
    struct remove_at<Axis, Cur, shape_tag<>> {
        using type = shape_tag<>;
    };
    ...
    struct remove_at<Axis, Cur, shape_tag<Head, Tail...>> {
        using type = std::conditional_t<
            Cur == Axis,
            shape_tag<Tail...>,
            typename prepend<Head, typename remove_at<Axis, Cur + 1,
                                                      shape_tag<Tail...>>::type>::type>;
    };
    ...
    template <>
    struct squeeze_tag<shape_tag<>> {
        using type = shape_tag<>;
    };

    template <int Head, int... Tail>
    struct squeeze_tag<shape_tag<Head, Tail...>> {
        using rest = typename squeeze_tag<shape_tag<Tail...>>::type;
        using type = std::conditional_t<Head == 1, rest,
     ...
     */
    template <int Value, int Axis, int Cur, typename Tag>
    struct insert;

    template <int Value, int Axis, int Cur>
    struct insert<Value, Axis, Cur, shape_tag<>> {
    ...
    struct insert<Value, Axis, Cur, shape_tag<Head, Tail...>> {
        using rest = typename insert<Value, Axis, Cur + 1,
                                     shape_tag<Tail...>>::type;
        using type = std::conditional_t<Cur == Axis,
                                        shape_tag<Value, Head, Tail...>,
                                        typename prepend<Head, rest>::type>;
    };
    ...
    template <int Head, int... Tail>
    struct head<shape_tag<Head, Tail...>> {
    ...
    template <int Head, int... Tail>
    struct tail<shape_tag<Head, Tail...>> {
        using type = shape_tag<Tail...>;
    };

    /** @brief True when two shape_tags are identical. */
    template <typename A, typename B>
    struct same_tag : std::false_type {};

    template <int... A>
    struct same_tag<shape_tag<A...>, shape_tag<A...>> : std::true_type {};
