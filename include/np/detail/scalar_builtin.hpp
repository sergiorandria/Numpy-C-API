...
template <typename T>
struct is_complex_instance<std::complex<T>> : std::true_type {};
template <typename T>
inline constexpr bool is_complex_instance_v = is_complex_instance<T>::value;

/** @brief True for a plain scalar element type (arithmetic / complex). */
template <typename T>
inline constexpr bool is_plain_scalar_v =
    std::is_arithmetic_v<T> || is_complex_instance_v<T>;
...
template <typename T> struct scalar_traits {
  ...
  static constexpr value_type make(const value_type &v) noexcept { return v; }
...
template <typename T>
using scalar_core_t = typename scalar_traits<std::remove_cv_t<T>>::value_type;
...
template <typename Op, typename A, typename B,
          bool Custom = needs_custom_kernel<A, B>>
struct binary_apply {
  /** @brief Stored element type the expression materializes. */
  using type = std::invoke_result_t<Op, A, B>;

  static constexpr type call(const A &a, const B &b) { return Op{}(a, b); }
};

/** @brief Builtin unary evaluation (stored value passes through). */
template <typename Op, typename A, bool Custom = scalar_traits<A>::is_custom>
struct unary_apply {
  /** @brief Stored element type the expression materializes. */
  using type = std::invoke_result_t<Op, A>;

  static constexpr type call(const A &a) { return Op{}(a); }
};
