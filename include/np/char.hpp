 ...
 *  - split(), rsplit(), splitlines() return flattened arrays (not object arrays)
 *  - translate() uses simplified 256-character table (not dict-based)
 *  - chararray class is deprecated (per NumPy 2.5) but provided for compatibility
 *
...
namespace ch {
     ...
     */
    NP_API inline auto mod(const Ndarray<std::string>& a, const Ndarray<std::string>& values)
        -> Ndarray<std::string> {
     ...
     */
    NP_API inline auto center(const Ndarray<std::string>& a, int width, char fillchar = ' ')
        -> Ndarray<std::string> {
        Ndarray<std::string> result = empty<std::string>(a.shape);
        for (std::size_t i = 0; i < a.size(); ++i) {
     ...
     */
    NP_API inline auto ljust(const Ndarray<std::string>& a, int width, char fillchar = ' ')
        -> Ndarray<std::string> {
        Ndarray<std::string> result = empty<std::string>(a.shape);
        for (std::size_t i = 0; i < a.size(); ++i) {
            const std::string& s = a.data()[i];
     ...
     */
    NP_API inline auto rjust(const Ndarray<std::string>& a, int width, char fillchar = ' ')
        -> Ndarray<std::string> {
        Ndarray<std::string> result = empty<std::string>(a.shape);
        for (std::size_t i = 0; i < a.size(); ++i) {
            const std::string& s = a.data()[i];
     ...
     */
    NP_API inline auto replace(const Ndarray<std::string>& a, const std::string& old,
                                const std::string& new_str, int count = -1)
        -> Ndarray<std::string> {
        Ndarray<std::string> result = empty<std::string>(a.shape);
        for (std::size_t i = 0; i < a.size(); ++i) {
            std::string s = a.data()[i];
     ...
     */
    NP_API inline auto equal(const Ndarray<std::string>& x1, const Ndarray<std::string>& x2)
        -> Ndarray<bool> {
     ...
     * @return Boolean array
     */
    NP_API inline auto not_equal(const Ndarray<std::string>& x1, const Ndarray<std::string>& x2)
        -> Ndarray<bool> {
        if (x1.shape != x2.shape) {
     ...
     * @return Boolean array
     */
    NP_API inline auto greater_equal(const Ndarray<std::string>& x1, const Ndarray<std::string>& x2)
        -> Ndarray<bool> {
        if (x1.shape != x2.shape) {
     ...
     * @return Boolean array
     */
    NP_API inline auto less_equal(const Ndarray<std::string>& x1, const Ndarray<std::string>& x2)
        -> Ndarray<bool> {
        if (x1.shape != x2.shape) {
     ...
     * @return Boolean array
     */
    NP_API inline auto greater(const Ndarray<std::string>& x1, const Ndarray<std::string>& x2)
        -> Ndarray<bool> {
        if (x1.shape != x2.shape) {
     ...
     * @return Boolean array
     */
    NP_API inline auto less(const Ndarray<std::string>& x1, const Ndarray<std::string>& x2)
        -> Ndarray<bool> {
        if (x1.shape != x2.shape) {
    ...
    /**
     * @brief Returns a boolean array which is True where the string element ends with suffix.
     *
     ...
     * @return Boolean array
     */
    NP_API inline auto endswith(const Ndarray<std::string>& a, const std::string& suffix,
                                 int start = 0, int end = -1)
        -> Ndarray<bool> {
        ...
        for (std::size_t i = 0; i < a.size(); ++i) {
            ...
            result.data()[i] = (sub.size() >= suffix.size() &&
    ...
    /**
     * @brief Returns a boolean array which is True where the string element starts with prefix.
     *
     ...
     * @return Boolean array
     */
    NP_API inline auto startswith(const Ndarray<std::string>& a, const std::string& prefix,
                                   int start = 0, int end = -1)
        -> Ndarray<bool> {
        Ndarray<bool> result = empty<bool>(a.shape);
        for (std::size_t i = 0; i < a.size(); ++i) {
     ...
     */
    NP_API inline auto str_len(const Ndarray<std::string>& a)
        -> Ndarray<int> {
        Ndarray<int> result = empty<int>(a.shape);
        for (std::size_t i = 0; i < a.size(); ++i) {
     ...
     * @return Boolean array
     */
    NP_API inline auto isalpha(const Ndarray<std::string>& a)
        -> Ndarray<bool> {
        Ndarray<bool> result = empty<bool>(a.shape);
        for (std::size_t i = 0; i < a.size(); ++i) {
            const std::string& s = a.data()[i];
            result.data()[i] = !s.empty() && std::all_of(s.begin(), s.end(),
     ...
     * @return Boolean array
     */
    NP_API inline auto isalnum(const Ndarray<std::string>& a)
        -> Ndarray<bool> {
        Ndarray<bool> result = empty<bool>(a.shape);
        for (std::size_t i = 0; i < a.size(); ++i) {
            const std::string& s = a.data()[i];
     ...
     * @return Boolean array
     */
    NP_API inline auto isdigit(const Ndarray<std::string>& a)
        -> Ndarray<bool> {
        Ndarray<bool> result = empty<bool>(a.shape);
        for (std::size_t i = 0; i < a.size(); ++i) {
            const std::string& s = a.data()[i];
     ...
     * @return Boolean array
     */
    NP_API inline auto islower(const Ndarray<std::string>& a)
        -> Ndarray<bool> {
        Ndarray<bool> result = empty<bool>(a.shape);
        for (std::size_t i = 0; i < a.size(); ++i) {
            const std::string& s = a.data()[i];
     ...
     * @return Boolean array
     */
    NP_API inline auto isupper(const Ndarray<std::string>& a)
        -> Ndarray<bool> {
        Ndarray<bool> result = empty<bool>(a.shape);
        for (std::size_t i = 0; i < a.size(); ++i) {
            const std::string& s = a.data()[i];
     ...
     * @return Boolean array
     */
    NP_API inline auto isspace(const Ndarray<std::string>& a)
        -> Ndarray<bool> {
        Ndarray<bool> result = empty<bool>(a.shape);
        for (std::size_t i = 0; i < a.size(); ++i) {
            const std::string& s = a.data()[i];
     ...
     * @return Boolean array
     */
    NP_API inline auto istitle(const Ndarray<std::string>& a)
        -> Ndarray<bool> {
        Ndarray<bool> result = empty<bool>(a.shape);
        for (std::size_t i = 0; i < a.size(); ++i) {
            const std::string& s = a.data()[i];
     ...
     * @return Boolean array
     */
    NP_API inline auto isdecimal(const Ndarray<std::string>& a)
        -> Ndarray<bool> {
        Ndarray<bool> result = empty<bool>(a.shape);
        for (std::size_t i = 0; i < a.size(); ++i) {
            const std::string& s = a.data()[i];
     ...
     * @return Boolean array
     */
    NP_API inline auto isnumeric(const Ndarray<std::string>& a)
        -> Ndarray<bool> {
        Ndarray<bool> result = empty<bool>(a.shape);
        for (std::size_t i = 0; i < a.size(); ++i) {
            const std::string& s = a.data()[i];
     ...
     */
    NP_API inline auto join(const Ndarray<std::string>& sep, const Ndarray<std::string>& seq)
        -> Ndarray<std::string> {
    ...
    NP_API inline auto partition(const Ndarray<std::string>& a, const std::string& sep)
        -> Ndarray<std::string> {
        // Returns array with 3x elements: [before0, sep0, after0, before1, sep1, after1, ...]
     ...
     *
     * Note: Returns flattened array of all split parts. In Python numpy, this returns
     * object arrays of lists. C++ implementation returns concatenated results.
     *
     * @param a Input string array
     ...
     */
    NP_API inline auto split(const Ndarray<std::string>& a, const std::string& sep = "",
                              int maxsplit = -1)
        -> Ndarray<std::string> {
        ...
        for (std::size_t i = 0; i < a.size(); ++i) {
            ...
            if (sep.empty()) {
                ...
                while (iss >> word) {
                    ...
                    if (maxsplit >= 0 && static_cast<int>(parts.size()) > maxsplit) break;
                }
            } else {
                // Split on separator
                std::size_t start = 0;
        ...
        Ndarray<std::string> result = empty<std::string>(std::vector<int>{static_cast<int>(all_parts.size())});
     ...
     * @return Flattened array of split strings
     */
    NP_API inline auto rsplit(const Ndarray<std::string>& a, const std::string& sep = "",
                               int maxsplit = -1)
        -> Ndarray<std::string> {
        std::vector<std::string> all_parts;
    ...
    NP_API inline auto splitlines(const Ndarray<std::string>& a, bool keepends = false)
        ...
        Ndarray<std::string> result = empty<std::string>(std::vector<int>{static_cast<int>(all_lines.size())});
     ...
     *
     * Simplified implementation: table is a string of 256 characters for direct mapping.
     *
     * @param a Input string array
    ...
    NP_API inline auto encode(const Ndarray<std::string>& a,
        ...
        -> Ndarray<std::string> {
        (void)encoding; (void)errors; // Unused in C++
     ...
     *
     * Reference: numpy-reference/reference/generated/numpy.char.compare_chararrays.html
     *
     * @param x1 First string array
     * @param x2 Second string array
    ...
    public:
        ...
        auto split(const std::string& sep = "", int maxsplit = -1) const -> Ndarray<std::string>
            { return ch::split(data_, sep, maxsplit); }
        auto rsplit(const std::string& sep = "", int maxsplit = -1) const -> Ndarray<std::string>
