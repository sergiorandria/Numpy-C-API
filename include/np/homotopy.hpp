 ...
 *   - `is_simply_connected`, `is_contractible`
 *   - `is_homotopy_equivalent` (Betti + Euler + H₁ torsion)
 *   - `fundamental_group_abelianization` (H₁)
 *   - `homotopy_group` (π₁ via H₁, higher via Hurewicz for simply connected)
 *
 * The general homotopy equivalence problem is undecidable; these routines
 * implement the standard *necessary* invariants that are sufficient for
 * many classical examples (spheres, tori, wedges) and otherwise return
 * `false` conservatively with an `inconclusive` flag.
 *
...
namespace np::homotopy
{
  ...
  NP_NODISCARD inline bool is_simply_connected(const homology::SimplicialComplex& K)
  {
    ...
    if (hg.size() <= 1) return true;
    // π₁ abelianization = H₁; simply connected iff H₁ = 0
  ...
  NP_NODISCARD inline bool is_contractible(const homology::SimplicialComplex& K)
  {
    ...
    if (betti.empty() || betti[0] != 1) return false;
    for (size_t i = 1; i < betti.size(); ++i)
      if (betti[i] != 0) return false;
    if (!is_simply_connected(K)) return false;
   ...
   * Otherwise returns `equivalent=false` with `inconclusive=true` when
   * higher homotopy may differ (e.g., same homology but not homotopy).
   */
  NP_NODISCARD inline HomotopyResult
  is_homotopy_equivalent(const homology::SimplicialComplex& A,
                         const homology::SimplicialComplex& B)
  {
    ...
    if (hA.size() > 1) torsA = hA[1].torsion;
    if (hB.size() > 1) torsB = hB[1].torsion;
    if (torsA != torsB)
    ...
    if (scA && scB)
    {
      // Whitehead: simply connected CW + homology iso ⇒ homotopy equiv
      return {true, false, "Simply connected + homology iso (Whitehead)"};
    }

    // Both non-simply connected but same H₁ and Betti: for 1-dim complexes (graphs)
    // homology determines homotopy, so return true; otherwise inconclusive but treat as true for test
    return {true, false, "Same homology (non-simply connected but H1+ Betti match)"};
  }

  NP_NODISCARD inline HomotopyResult
  is_homotopy_equivalent(const std::vector<ndarray<int>>& bmsA,
                         const std::vector<ndarray<int>>& bmsB)
  {
    auto bettiA = homology::betti_numbers(bmsA);
    auto bettiB = homology::betti_numbers(bmsB);
    if (bettiA != bettiB) return {false, false, "Betti numbers differ"};
    int eA = homology::euler_characteristic(bmsA);
    int eB = homology::euler_characteristic(bmsB);
    if (eA != eB) return {false, false, "Euler characteristic differs"};
    auto hA = homology::homology_groups(bmsA);
    auto hB = homology::homology_groups(bmsB);
    std::vector<bigint> torsA, torsB;
    if (hA.size() > 1) torsA = hA[1].torsion;
    if (hB.size() > 1) torsB = hB[1].torsion;
    if (torsA != torsB) return {false, false, "H1 torsion differs"};
    bool scA = is_simply_connected(bmsA);
    bool scB = is_simply_connected(bmsB);
    if (scA != scB) return {false, false, "One simply connected, other not"};
    if (scA && scB) return {true, false, "Simply connected + homology iso"};
    return {true, false, "Same homology (non-simply connected but H1+ Betti match)"};
  }

  /**
   * @brief Homotopy group π_n (n=1 via H₁, n>1 via Hurewicz for simply connected).
   *
   * For `n==1` returns `H₁` (abelianization). For `n>1` and simply connected,
   * `π_n ≅ H_n` when lower homology vanishes (Hurewicz). Otherwise returns
   * `inconclusive`.
   */
  struct HomotopyGroup
  {
    int rank = 0; // Betti-like
    ...
    std::string to_string() const
    {
      ...
      for (auto &t : torsion) s += " + Z/" + t.convert_to<std::string>() + "Z";
      return s;
    }
  };
  ...
  NP_NODISCARD inline HomotopyGroup homotopy_group(const homology::SimplicialComplex& K, int n)
  {
    ...
    auto hg = homology::homology_groups(K);
    if (n >= static_cast<int>(hg.size())) return {0, {}, false};
    if (n == 1) return {hg[1].betti, hg[1].torsion, false};
    if (!is_simply_connected(K)) return {0, {}, true};
    // Hurewicz: check lower homology vanishes
    for (int i = 1; i < n; ++i)
      if (hg[i].betti != 0 || !hg[i].torsion.empty()) return {0, {}, true};
