...
namespace np
{
  ...
  namespace detail
  {
    ...
    auto
    elementwise_into(const ndarray<T>& a, const ndarray<U>& b, ndarray<R>& out, Fn&& fn)
        -> ndarray<R>&
    {
    ...
    NP_NODISCARD auto
    ufunc_ternary(const ndarray<T>& a, const ndarray<U>& b, const ndarray<V>& c, Fn&& fn)
    {
     ...
     */
    template <typename T>
    inline T roundto_elem(T v, int decimals)
    {
      ...
      -> ndarray<std::common_type_t<T, U>>
  {
    ...
    return detail::elementwise(
        x1, x2, [](const T& y, const U& x) { return static_cast<R>(std::atan2(y, x)); });
  }
  ...
  NP_API template <detail::Arithmetic T, detail::Arithmetic U>
  auto arctan2(
      const ndarray<T>& x1, const ndarray<U>& x2, ndarray<std::common_type_t<T, U>>& out)
      -> ndarray<std::common_type_t<T, U>>&
  {
    using R = std::common_type_t<T, U>;
    return detail::elementwise_into(
        x1,
        x2,
        out,
      ...
      -> ndarray<std::common_type_t<T, U>>
  {
    using R = std::common_type_t<T, U>;
    return detail::elementwise(
        x1, x2, [](const T& a, const U& b) { return static_cast<R>(std::hypot(a, b)); });
  }
  ...
  NP_NODISCARD auto square(const ndarray<T>& x) -> ndarray<T>
  {
    if constexpr (std::is_same_v<T, float> || std::is_same_v<T, double>)
    {
      ...
      if (x.is_contiguous())
      {
        ...
        if (result.size() > 0)
        {
          np::simd::mul_vectorized(
              x.data().data(), x.data().data(), result.data().data(), result.size());
        }
        return result;
      }
    }
  ...
  auto square(const ndarray<T>& x, ndarray<T>& out) -> ndarray<T>&
  {
    if constexpr (std::is_same_v<T, float> || std::is_same_v<T, double>)
    {
      if (x.is_contiguous() && out.is_contiguous() && out.shape == x.shape)
      {
        ...
        {
          np::simd::mul_vectorized(
              x.data().data(), x.data().data(), out.data().data(), out.size());
        }
        return out;
      }
    }
  ...
  auto round(const ndarray<T>& x, int decimals, ndarray<T>& out) -> ndarray<T>&
  {
    return detail::ufunc_unary_into(
        x,
        out,
        [decimals](const T& v) -> T
        {
          if constexpr (detail::is_complex_v<T>)
          {
            return T{
                detail::roundto_elem(v.real(), decimals),
      ...
      -> ndarray<std::common_type_t<T, U>>
  {
    using R = std::common_type_t<T, U>;
    return detail::elementwise(
        x1, x2, [](const T& a, const U& b) { return static_cast<R>(std::max(a, b)); });
  }
      ...
      -> ndarray<std::common_type_t<T, U>>&
  {
    ...
    return detail::elementwise_into(
        ...
        x2,
        out,
        [](const T& a, const U& b) { return static_cast<R>(std::max(a, b)); });
  }
      ...
      -> ndarray<std::common_type_t<T, U>>
  {
    using R = std::common_type_t<T, U>;
    return detail::elementwise(
        x1, x2, [](const T& a, const U& b) { return static_cast<R>(std::min(a, b)); });
  }
      ...
      -> ndarray<std::common_type_t<T, U>>&
  {
    ...
    return detail::elementwise_into(
        ...
        x2,
        out,
        [](const T& a, const U& b) { return static_cast<R>(std::min(a, b)); });
  }
      ...
      -> ndarray<std::common_type_t<T, U>>
  {
    using R = std::common_type_t<T, U>;
    return detail::elementwise(
        x1, x2, [](const T& a, const U& b) { return static_cast<R>(std::fmax(a, b)); });
  }
  ...
  NP_API template <detail::Arithmetic T, detail::Arithmetic U>
  auto
  fmax(const ndarray<T>& x1, const ndarray<U>& x2, ndarray<std::common_type_t<T, U>>& out)
      -> ndarray<std::common_type_t<T, U>>&
  {
    using R = std::common_type_t<T, U>;
    return detail::elementwise_into(
      ...
      -> ndarray<std::common_type_t<T, U>>
  {
    using R = std::common_type_t<T, U>;
    return detail::elementwise(
        x1, x2, [](const T& a, const U& b) { return static_cast<R>(std::fmin(a, b)); });
  }
  ...
  NP_API template <detail::Arithmetic T, detail::Arithmetic U>
  auto
  fmin(const ndarray<T>& x1, const ndarray<U>& x2, ndarray<std::common_type_t<T, U>>& out)
      -> ndarray<std::common_type_t<T, U>>&
  {
    using R = std::common_type_t<T, U>;
    return detail::elementwise_into(
      ...
      -> ndarray<std::common_type_t<T, U>>
  {
    using R = std::common_type_t<T, U>;
    return detail::elementwise(
        x1, x2, [](const T& a, const U& b) { return static_cast<R>(std::fmod(a, b)); });
  }
  ...
  NP_API template <detail::Arithmetic T, detail::Arithmetic U>
  auto
  fmod(const ndarray<T>& x1, const ndarray<U>& x2, ndarray<std::common_type_t<T, U>>& out)
      -> ndarray<std::common_type_t<T, U>>&
  {
    using R = std::common_type_t<T, U>;
    return detail::elementwise_into(
  ...
  NP_API template <detail::Arithmetic T, detail::Arithmetic U>
  auto
  mod(const ndarray<T>& x1, const ndarray<U>& x2, ndarray<std::common_type_t<T, U>>& out)
      -> ndarray<std::common_type_t<T, U>>&
  {
      ...
      -> ndarray<std::common_type_t<T, U>>
  {
    using R = std::common_type_t<T, U>;
    if constexpr (
        std::is_same_v<T, U> && (std::is_same_v<T, float> || std::is_same_v<T, double>))
    {
      if (x1.is_contiguous() && x2.is_contiguous() && x1.shape == x2.shape)
      {
        ...
        if (result.size() > 0)
        {
          np::simd::div_vectorized(
              x1.data().data(), x2.data().data(), result.data().data(), result.size());
        }
        return result;
      }
    }
      ...
      -> ndarray<std::common_type_t<T, U>>&
  {
        ...
        std::is_same_v<T, U> && (std::is_same_v<T, float> || std::is_same_v<T, double>))
    {
          ...
          && out.shape == x1.shape && x1.shape == x2.shape)
      {
        ...
        {
          np::simd::div_vectorized(
              x1.data().data(), x2.data().data(), out.data().data(), out.size());
        }
        return out;
      }
    }
    return detail::elementwise_into(
        x1, x2, out, [](const T& a, const U& b) { return static_cast<R>(a / b); });
  }
  ...
  {
    return detail::elementwise(
        x1, x2, [](const T& a, const U& b) { return detail::floored_div(a, b); });
  }
  ...
  NP_NODISCARD auto clip(const ndarray<T>& x, const T& a_min, const T& a_max)
      -> ndarray<T>
  {
      ...
      -> ndarray<std::common_type_t<T, U, V>>
  {
    ...
    return detail::ufunc_ternary(
        x1,
        x2,
        x3,
        [](const T& a, const U& b, const V& c) -> R
        {
  ...
  NP_API template <detail::Numeric T, detail::Numeric U, detail::Numeric V>
  auto
  fma(const ndarray<T>& x1,
      const ndarray<U>& x2,
      const ndarray<V>& x3,
      ndarray<std::common_type_t<T, U, V>>& out) -> ndarray<std::common_type_t<T, U, V>>&
  {
    using R = std::common_type_t<T, U, V>;
    return detail::ufunc_ternary_into(
        x1,
        x2,
        x3,
        out,
  ...
  NP_API template <detail::FloatingPoint T>
  NP_NODISCARD auto
  unwrap(const ndarray<T>& p, T discont = std::numbers::pi_v<T>, int axis = -1)
      -> ndarray<T>
  {
    if (p.ndim() != 1)
    {
  ...
  NP_NODISCARD auto ediff1d(const ndarray<T>& ary) -> ndarray<T>
  {
    ...
    {
      out.data()[i] =
          ary.data()[ary._flat_logical(i + 1)] - ary.data()[ary._flat_logical(i)];
    }
    return out;
  }
      ...
      -> ndarray<std::common_type_t<T, U>>
  {
    using R = std::common_type_t<T, U>;
    return detail::elementwise(
        a, b, [](const T& x, const U& y) -> R { return static_cast<R>(x + y); });
  }
  NP_API template <detail::Numeric T, detail::Numeric U>
  auto
  add(const ndarray<T>& a, const ndarray<U>& b, ndarray<std::common_type_t<T, U>>& out)
      -> ndarray<std::common_type_t<T, U>>&
  {
    using R = std::common_type_t<T, U>;
    return detail::elementwise_into(
        a, b, out, [](const T& x, const U& y) -> R { return static_cast<R>(x + y); });
  }
  ...
  NP_API template <detail::Numeric T, detail::Numeric U>
  auto subtract(
      const ndarray<T>& a, const ndarray<U>& b, ndarray<std::common_type_t<T, U>>& out)
      -> ndarray<std::common_type_t<T, U>>&
  {
    using R = std::common_type_t<T, U>;
    return detail::elementwise_into(
  ...
  {
    static_assert(
        std::is_integral_v<T> && std::is_integral_v<U>, "gcd requires integral types");
    using R = std::common_type_t<T, U>;
    return detail::elementwise(
        a,
        b,
        [](const T& x, const U& y) -> R
        { return std::gcd(static_cast<R>(x), static_cast<R>(y)); });
  }
  ...
  {
    static_assert(
        std::is_integral_v<T> && std::is_integral_v<U>, "lcm requires integral types");
    using R = std::common_type_t<T, U>;
    return detail::elementwise(
        a,
        b,
        [](const T& x, const U& y) -> R
        { return std::lcm(static_cast<R>(x), static_cast<R>(y)); });
  }
  ...
  NP_API template <detail::Numeric T, detail::Numeric U>
  NP_NODISCARD auto
  convolve(const ndarray<T>& a, const ndarray<U>& v, const std::string& mode = "full")
      -> ndarray<std::common_type_t<T, U>>
  {
  ...
  NP_API template <detail::FloatingPoint T>
  NP_NODISCARD auto interp(
      const ndarray<T>& x,
      const ndarray<T>& xp,
      const ndarray<T>& fp,
      T left = std::numeric_limits<T>::quiet_NaN(),
      T right = std::numeric_limits<T>::quiet_NaN()) -> ndarray<T>
  {
    ...
    for (std::size_t i = 0; i < x.size(); ++i)
    {
      ...
      {
        out.data()[i] =
            std::isnan(right) ? fp.data()[fp._flat_logical(fp.size() - 1)] : right;
        continue;
      }
      // binary search
  ...
  {
    return detail::elementwise(
        x1, x2, [](const T& a, const int& b) -> T { return std::ldexp(a, b); });
  }

  /** @brief Discrete difference (np.diff) – n-th order along axis. */
  NP_API template <detail::Numeric T>
  NP_NODISCARD auto diff(const ndarray<T>& a, std::size_t n = 1, int axis = -1)
      -> ndarray<T>
  {
    if (n == 0)
      return a.copy();
  ...
  NP_NODISCARD auto gradient(const ndarray<T>& f, T dx = T{1}) -> ndarray<T>
  {
    ...
    out.data()[out._flat_logical(0)] =
        (f.data()[f._flat_logical(1)] - f.data()[f._flat_logical(0)]) / dx;
    out.data()[out._flat_logical(n - 1)] =
        (f.data()[f._flat_logical(n - 1)] - f.data()[f._flat_logical(n - 2)]) / dx;
    for (std::size_t i = 1; i + 1 < n; ++i)
    {
      out.data()[out._flat_logical(i)] =
          (f.data()[f._flat_logical(i + 1)] - f.data()[f._flat_logical(i - 1)])
          / (T{2} * dx);
    }
    return out;
  }
  ...
  NP_API template <detail::Numeric T>
  NP_NODISCARD auto trapz(const ndarray<T>& y, T dx = T{1})
      -> std::conditional_t<(std::is_same_v<T, bool>), double, T>
  {
    using R = std::conditional_t<std::is_same_v<T, bool>, double, T>;
    if (y.ndim() != 1)
      throw std::invalid_argument(
          "trapz: only 1-D supported in this overload, use trapz(y,dx,axis) for ND");
    if (y.size() < 2)
      return R{0};
    R sum = R{0};
      ...
      -> std::conditional_t<(std::is_same_v<T, bool>), double, T>
  {
    ...
    if (y.ndim() == 1)
    {
      ...
      for (std::size_t i = 0; i + 1 < y.size(); ++i)
      {
        ...
        R y1 = static_cast<R>(y.data()[y._flat_logical(i + 1)]);
        R dx = static_cast<R>(
            x.data()[x._flat_logical(i + 1)] - x.data()[x._flat_logical(i)]);
        sum += (y0 + y1) * dx / R{2};
      }
      return sum;
    }
