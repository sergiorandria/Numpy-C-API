...
NP_NODISCARD auto lexsort(const std::vector<ndarray<T>> &keys)
    -> ndarray<std::size_t> {
  if (keys.empty()) {
  ...
  for (auto &k : keys) {
    if (k.ndim() != 1) throw std::invalid_argument("lexsort: keys must be 1-D");
    if (k.size() != n) throw std::invalid_argument("lexsort: keys length mismatch");
  }
  ...
  for (std::size_t i = 0; i < n; ++i) out.data()[i] = idx[i];
  return out;
}
...
NP_NODISCARD auto msort(const ndarray<T> &arr) -> ndarray<T> {
  if (arr.ndim() <= 1) return arr.sorted();
  return arr.sorted(0);
}
 ...
 */
NP_API template <typename T>
NP_NODISCARD auto sort_complex(const ndarray<T> &arr)
    -> ndarray<std::complex<double>> {
  // Promote to complex<double>
  ndarray<std::complex<double>> c(arr.shape);
  for (std::size_t i = 0; i < arr.size(); ++i) {
    c.data()[c._flat_logical(i)] =
        std::complex<double>(static_cast<double>(arr.data()[arr._flat_logical(i)]), 0.0);
  }
  // sort flattened then reshape? numpy sorts along last axis; we mirror member sort logic: sort last axis
  c.sort(-1);
  // For complex, we need real-then-imag comparison; std::sort uses operator< which does lex order exactly.
  return c;
}
...
NP_NODISCARD auto flatnonzero(const ndarray<T> &arr) -> ndarray<std::size_t> {
  ...
  for (auto it = arr.begin(); it != arr.end(); ++it, ++flat) {
    if (static_cast<bool>(*it)) idx.push_back(flat);
  }
  ndarray<std::size_t> out(std::vector<int>{static_cast<int>(idx.size())});
  for (std::size_t i = 0; i < idx.size(); ++i) out.data()[i] = idx[i];
  return out;
}
...
NP_NODISCARD std::size_t count_nonzero(const ndarray<T> &arr) {
  ...
  for (auto it = arr.begin(); it != arr.end(); ++it) {
    if (static_cast<bool>(*it)) ++c;
  }
  return c;
}

NP_API template <typename T>
NP_NODISCARD auto count_nonzero(const ndarray<T> &arr, int axis)
    -> ndarray<std::size_t> {
  axis = static_cast<int>(arr.ndim()) > 0 ? [&]{
    int a = axis;
    if (a < 0) a += static_cast<int>(arr.ndim());
    if (a < 0 || a >= static_cast<int>(arr.ndim())) throw AxisError("count_nonzero: axis out of bounds");
    return a;
  ...
  while (!od.done()) {
    ...
    if (static_cast<bool>(arr.get(idx))) {
      ...
      for (std::size_t d = 0; d < idx.size(); ++d) if (static_cast<int>(d) != axis) oidx.push_back(idx[d]);
