...
namespace np::linalg {
    ...
    namespace detail {
        ...
        std::vector<R> dense2d(const Ndarray<T>& a,
        ...
        template <typename R>
        Ndarray<R> mk2d(std::size_t rows, std::size_t cols,
                        std::vector<R>&& data) {
        ...
        template <typename R>
        std::vector<R> take_cols(const std::vector<R>& data,
                                 std::size_t rows, std::size_t cols,
                                 std::size_t k) {
        ...
        template <typename R>
        void ortho_complete(std::vector<R>& data,
                            std::size_t rows, std::size_t cols) {
            for (std::size_t j = 0; j < cols; ++j) {
                R nrm{};
                for (std::size_t i = 0; i < rows; ++i) {
        ...
        template <typename R>
        void svd_jacobi(const std::vector<R>& a,
                        std::size_t p, std::size_t q,
                        bool full, bool want_uv,
                        std::vector<R>& u, std::vector<R>& s, std::vector<R>& v) {
            ...
            std::sort(order.begin(), order.end(),
                      [&norms](std::size_t x, std::size_t y) {
        ...
        // numpy's raw mode.
        template <typename R>
        void householder_qr(const std::vector<R>& a,
                            std::size_t m, std::size_t n,
                            std::vector<R>& q, std::vector<R>& r,
        ...
        template <typename R>
        void francis_schur(std::vector<R>& h, std::size_t n,
                           std::vector<R>& q) {
            if (n < 2) {
                return;
            }
            ...
            while (p > 0) {
                ...
                std::ptrdiff_t l = p;
                while (l > 0) {
                    const R tol = eps * (std::abs(h[(l - 1) * n + l - 1]) +
                                         std::abs(h[l * n + l]));
        ...
        template <typename R>
        std::vector<std::complex<R>> schur_eigenvalues(
            const std::vector<R>& h, std::size_t n) {
        ...
        std::vector<std::complex<R>> schur_eigenvectors(
            const std::vector<R>& h, const std::vector<R>& q, std::size_t n,
            ...
            for (std::size_t j = 0; j < n; ++j) {
                ...
                for (std::ptrdiff_t ii = static_cast<std::ptrdiff_t>(n) - 1;
                     ii >= 0; --ii) {
        ...
        template <typename R>
        void schur_decompose(const std::vector<R>& a, std::size_t n,
                             std::vector<R>& schur, std::vector<R>& q) {
            schur = a;
        ...
        template <typename R>
        bool lu_factor(std::vector<R>& a, std::size_t n,
                       std::vector<std::size_t>& piv, std::size_t& swaps) {
            piv.resize(n);
            swaps = 0;
        ...
        template <typename R>
        std::vector<R> lu_solve(const std::vector<R>& lu, std::size_t n,
        ...
        template <typename R>
        std::vector<R> lu_invert(const std::vector<R>& lu, std::size_t n,
                                 const std::vector<std::size_t>& piv) {
        ...
        inline std::vector<std::size_t> c_order_strides(
    ...
    // complete, r, raw.
    enum class QrMode { Reduced, Complete, R, Raw };
    ...
    auto svd(const Ndarray<T>& a, bool full_matrices = true,
             bool compute_uv = true) -> SVDResult<real_t<T>> {
        using R = real_t<T>;
        if (a.ndim() != 2) {
        ...
        detail::svd_jacobi(dense, m, n, full_matrices, compute_uv,
                           u_vec, s_vec, v_vec);
        out.s = Ndarray<R>::from_data(
            std::vector<int>{static_cast<int>(k)}, std::move(s_vec));
        if (!compute_uv) {
        ...
        if (m >= n) {
            ...
            detail::svd_jacobi(dense_t, n, m, full_matrices, compute_uv,
                               u_vec, s_vec, v_vec);
    ...
    auto qr(const Ndarray<T>& a, QrMode mode = QrMode::Reduced)
        ...
        switch (mode) {
            case QrMode::Reduced:
                out.q = detail::mk2d(m, k,
                                     detail::take_cols(q_vec, m, m, k));
            ...
            case QrMode::Raw:
                ...
                out.tau = Ndarray<R>::from_data(
                    std::vector<int>{static_cast<int>(k)},
                    std::move(tau_vec));
                break;
        }
        return out;
    ...
    auto eig(const Ndarray<T>& a) -> EigenResult<real_t<T>> {
        ...
        if (n == 1) {
            out.w = Ndarray<std::complex<R>>::from_data(
                std::vector<int>{1},
                std::vector<std::complex<R>>{{dense[0], R{0}}});
            out.v = Ndarray<std::complex<R>>::from_data(
                std::vector<int>{1, 1},
                std::vector<std::complex<R>>{{R{1}, R{0}}});
            return out;
        }
        ...
        std::vector<std::complex<R>> w =
            detail::schur_eigenvalues(schur, n);
        std::vector<std::complex<R>> v =
            detail::schur_eigenvectors(schur, q, n, w);
        out.w = Ndarray<std::complex<R>>::from_data(
    ...
    auto solve(const Ndarray<T>& a, const Ndarray<U>& b)
        ...
        if (b.ndim() == 0 || b.ndim() > 2) {
            throw std::invalid_argument(
                "solve requires b to be 1D or 2D");
        }
        std::size_t m{}, n{};
        std::vector<R> lu = detail::dense2d<R>(a, m, n);
        if (m != n) {
        ...
        const std::size_t nrhs =
            b.ndim() == 1 ? 1 : static_cast<std::size_t>(b.shape[1]);
        std::vector<R> out(m * nrhs);
        for (std::size_t c = 0; c < nrhs; ++c) {
            std::vector<R> rhs(m);
            for (std::size_t i = 0; i < m; ++i) {
                rhs[i] = b.ndim() == 1 ? static_cast<R>(b.at(i))
                                       : static_cast<R>(b.at(i, c));
            }
        ...
        return b.ndim() == 1
                   ? Ndarray<R>::from_data(
                         std::vector<int>{static_cast<int>(m)},
                         std::move(out))
        ...
        if (nd == 1) {
            ...
            switch (ord) {
                ...
                case NormOrd::NegInf: {
                    ...
                    return best == std::numeric_limits<R>::infinity() ? R{0}
                                                                       : best;
                }
                case NormOrd::NegOne: {
                    R acc{};
        ...
        switch (ord) {
            ...
            case NormOrd::NegOne: {
                ...
                for (std::size_t j = 0; j < cols; ++j) {
                    ...
                    best = ord == NormOrd::One ? std::max(best, acc)
                                               : std::min(best, acc);
                }
                return best == std::numeric_limits<R>::infinity() ? R{0}
                                                                   : best;
            ...
            case NormOrd::NegInf: {
                ...
                for (std::size_t i = 0; i < rows; ++i) {
                    ...
                    best = ord == NormOrd::Inf ? std::max(best, acc)
                                               : std::min(best, acc);
                }
                return best == std::numeric_limits<R>::infinity() ? R{0}
                                                                   : best;
    ...
    auto matrix_norm(const Ndarray<T>& x, NormOrd ord = NormOrd::Fro)
        ...
        if (x.ndim() != 2) {
            throw std::invalid_argument(
                "matrix_norm requires a 2D array");
        }
        if (ord == NormOrd::Nuc) {
            auto s = svdvals(x);
            R acc{};
    ...
    template <typename T>
        requires(!np::detail::is_complex_v<T>)
    auto vector_norm(const Ndarray<T>& x, const std::vector<int>& axis = {},
        ...
        if (axis.empty()) {
            ...
            for (int ax : axis) {
                ...
                if (r < 0 || r >= static_cast<int>(nd) || is_red[u]) {
                    throw std::invalid_argument(
                        "vector_norm: invalid or repeated axis");
                }
                is_red[u] = true;
        ...
        while (!odo.done()) {
            ...
            while (!rdone) {
                ...
                const R v = static_cast<R>(
                    std::abs(static_cast<R>(x.data()[x._flat(idx)])));
                if (is_inf) {
                    if (first) {
                        best = v;
                    } else {
                ...
                for (std::size_t d = ridx.size(); d-- > 0;) {
                    if (++ridx[d] <
                        static_cast<std::size_t>(x.shape[red_axes[d]])) {
                        rdone = false;
                        break;
                    }
                    ridx[d] = 0;
            ...
            if (is_inf) {
                ...
                val = static_cast<R>(
                    std::pow(static_cast<double>(acc), 1.0 / ord));
            }
    ...
    auto matrix_rank(const Ndarray<T>& a) -> int {
        ...
        if (nd > 2) {
            throw std::invalid_argument(
                "matrix_rank requires a 1D or 2D array");
        }
        if (nd == 1) {
        ...
        const R tol =
            s(0) * static_cast<R>(std::max(a.shape[0], a.shape[1])) *
    ...
    template <typename T>
        requires(!np::detail::is_complex_v<T>)
    auto matrix_rank(const Ndarray<T>& a, double tol) -> int {
        using R = real_t<T>;
        const std::size_t nd = a.ndim();
        if (nd > 2) {
            throw std::invalid_argument(
    ...
    template <typename T>
        requires(!np::detail::is_complex_v<T>)
    auto eigvalsh(const Ndarray<T>& a) -> Ndarray<real_t<T>> {
        using R = real_t<T>;
        auto w = eigvals(a);
        ...
        return Ndarray<R>::from_data(
            std::vector<int>{static_cast<int>(n)}, std::move(re));
    }
    ...
    template <typename T>
        requires(!np::detail::is_complex_v<T>)
    auto eigh(const Ndarray<T>& a) -> EighResult<real_t<T>> {
        using R = real_t<T>;
        auto e = eig(a);
        ...
        std::sort(order.begin(), order.end(),
                  [&e](std::size_t x, std::size_t y) {
                      return e.w(x).real() < e.w(y).real();
                  });
        std::vector<R> w(n);
        std::vector<R> v(n * n);
        for (std::size_t j = 0; j < n; ++j) {
    ...
    // Raises std::invalid_argument unless x.ndim() >= 2.
    template <typename T>
    auto diagonal(const Ndarray<T>& x, int offset = 0) -> Ndarray<T> {
        const std::size_t nd = x.ndim();
        if (nd < 2) {
            throw std::invalid_argument(
                "diagonal requires a 2D array or a stack");
        }
        ...
        while (!odo.done()) {
            ...
            in.resize(nd);
            for (int d = 0; d < len; ++d) {
                in[nd - 2] = static_cast<std::size_t>(
                    offset >= 0 ? d : d - offset);
                in[nd - 1] = static_cast<std::size_t>(
                    offset >= 0 ? d + offset : d);
                std::vector<std::size_t> oi(li.begin(), li.end());
                oi.push_back(static_cast<std::size_t>(d));
                out.data()[np::detail::flat_index(oi, out.strides, 0)] =
                    x.data()[x._flat(in)];
            }
            odo.advance();
        }
    ...
    // Reference: numpy-reference/reference/generated/numpy.linalg.matrix_transpose.html
    // Raises std::invalid_argument unless x.ndim() >= 2.
    template <typename T>
    auto matrix_transpose(const Ndarray<T>& x) -> Ndarray<T> {
        const std::size_t nd = x.ndim();
        if (nd < 2) {
            throw std::invalid_argument(
                "matrix_transpose requires a 2D array or a stack");
        }
    ...
    template <typename T>
        requires(!np::detail::is_complex_v<T>)
    auto tensorinv(const Ndarray<T>& a, int ind = 2) -> Ndarray<real_t<T>> {
        using R = real_t<T>;
        const std::size_t nd = a.ndim();
        if (ind <= 0 || static_cast<std::size_t>(ind) > nd) {
            throw std::invalid_argument(
                "tensorinv: ind must be between 1 and the array rank");
        }
        ...
        if (p1 != p2) {
            throw np::exceptions::LinAlgError(
                "tensorinv: the tensor is not square");
        }
        ...
        while (!odo.done()) {
            const auto& oi = odo.idx();
            mat[np::detail::flat_index(oi, cst, 0)] =
                static_cast<R>(a.data()[a._flat(oi)]);
            odo.advance();
        }
        std::vector<std::size_t> piv;
        std::size_t swaps{};
    ...
    template <typename T, typename U>
        requires(!np::detail::is_complex_v<T> && !np::detail::is_complex_v<U>)
    auto tensorsolve(const Ndarray<T>& a, const Ndarray<U>& b,
        ...
        if (axes.empty()) {
            ...
            for (int ax : axes) {
                const int r =
                    ax < 0 ? ax + static_cast<int>(a.ndim()) : ax;
                const std::size_t u = static_cast<std::size_t>(r);
                if (r < 0 || r >= static_cast<int>(a.ndim()) || seen[u]) {
                    throw std::invalid_argument(
                        "tensorsolve: invalid or repeated axis");
                }
                seen[u] = true;
            }
        ...
        std::vector<int> qshape(a2_shape.begin() + static_cast<long>(nb),
                                a2_shape.end());
        std::size_t p1 = 1, p2 = 1;
        for (std::size_t i = 0; i < nb; ++i) {
        ...
        if (p1 != p2) {
            throw np::exceptions::LinAlgError(
                "tensorsolve: the tensor is not square");
        }
        ...
        while (!odo.done()) {
            ...
            mat[np::detail::flat_index(oi, cst, 0)] =
                static_cast<R>(a.data()[a._flat(in)]);
            odo.advance();
        }
        ...
        while (!odb.done()) {
            ...
            rhs[np::detail::flat_index(oi, cstb, 0)] =
                static_cast<R>(b.data()[b._flat(oi)]);
            odb.advance();
        }
        std::vector<std::size_t> piv;
        std::size_t swaps{};
    ...
    auto dot(const Ndarray<T>& a, const Ndarray<U>& b)
        ...
        if (na == 1 && nb == 1) {
            ...
            R acc{};
            for (std::size_t i = 0; i < static_cast<std::size_t>(ashape[0]);
                 ++i) {
        ...
        if (na == 2 && nb == 1) {
            ...
            for (std::size_t i = 0; i < rows; ++i) {
                ...
                for (std::size_t j = 0; j < k; ++j) {
                    acc += static_cast<R>(
                               a.get(std::array<std::size_t, 2>{i, j})) *
            ...
            for (std::size_t j = 0; j < cols; ++j) {
                ...
                for (std::size_t i = 0; i < k; ++i) {
                    acc += static_cast<R>(a.at(i)) *
                           static_cast<R>(
                               b.get(std::array<std::size_t, 2>{i, j}));
                }
                out.data()[j] = acc;
            }
            return out;
        ...
        for (std::size_t i = 0; i < rows; ++i) {
            for (std::size_t j = 0; j < cols; ++j) {
                ...
                for (std::size_t p = 0; p < k; ++p) {
                    acc += static_cast<R>(
                               a.get(std::array<std::size_t, 2>{i, p})) *
                           static_cast<R>(
                               b.get(std::array<std::size_t, 2>{p, j}));
                }
    ...
    template <typename T>
        requires(!np::detail::is_complex_v<T>)
    auto matrix_power(const Ndarray<T>& a, long long n)
        -> Ndarray<real_t<T>> {
        using R = real_t<T>;
        if (a.ndim() != 2) {
        ...
        const unsigned long long e =
            n > 0 ? static_cast<unsigned long long>(n)
    ...
    template <typename T, typename U>
        requires(!np::detail::is_complex_v<T> && !np::detail::is_complex_v<U>)
    auto lstsq(const Ndarray<T>& a, const Ndarray<U>& b,
        ...
        if (static_cast<std::size_t>(b.shape[0]) != m) {
            throw std::invalid_argument(
                "lstsq: b's leading dimension must match a");
        }
        LstsqResult<R> out;
        out.s = svdvals(a);
        const std::size_t k = out.s.size();
        const R eps = std::numeric_limits<R>::epsilon();
        const double rcond_eff =
        ...
        if (m > n && out.rank == static_cast<int>(n)) {
            ...
            for (std::size_t c = 0; c < nrhs; ++c) {
                R acc{};
                for (std::size_t i = 0; i < m; ++i) {
                    R r = b.ndim() == 1 ? static_cast<R>(b.at(i))
                                        : static_cast<R>(b.at(i, c));
                    for (std::size_t j = 0; j < n; ++j) {
                        r -= static_cast<R>(a.at(i, j)) *
                             (b.ndim() == 1 ? out.x(j) : out.x(j, c));
                    }
                    acc += r * r;
                }
                res[c] = acc;
            ...
            out.residuals = b.ndim() == 1
                                ? Ndarray<R>::from_data(
                                      std::vector<int>{1}, std::move(res))
                                : Ndarray<R>::from_data(
                                      std::vector<int>{nr}, std::move(res));
        } else {
            out.residuals = Ndarray<R>(std::vector<int>{0});
        }
        return out;
    ...
    template <typename T>
    auto multi_dot(const std::vector<Ndarray<T>>& arrays) -> Ndarray<T> {
        const std::size_t n = arrays.size();
        if (n < 2) {
            throw std::invalid_argument("multi_dot needs at least two arrays");
        }
        if (arrays[0].ndim() == 0 || arrays[0].ndim() > 2 ||
            arrays[n - 1].ndim() == 0 || arrays[n - 1].ndim() > 2) {
            throw std::invalid_argument(
                ...
                for (std::size_t k = i; k < j; ++k) {
                    const std::size_t c =
                        ...
                        static_cast<std::size_t>(dims[i]) *
                            static_cast<std::size_t>(dims[k + 1]) *
    ...
    template <typename T, typename U>
        requires(!np::detail::is_complex_v<T> && !np::detail::is_complex_v<U>)
    auto tensordot(const Ndarray<T>& a, const Ndarray<U>& b,
        ...
        if (a_axes.size() != b_axes.size()) {
            throw std::invalid_argument(
                "tensordot: the axes sequences must have equal length");
        }
        const std::size_t na = a.ndim();
        const std::size_t nb = b.ndim();
        ...
        if (has_dup(aa) || has_dup(ba)) {
            throw std::invalid_argument(
                "tensordot: repeated axes are not allowed");
        }
        for (std::size_t i = 0; i < aa.size(); ++i) {
            if (a.shape[aa[i]] != b.shape[ba[i]]) {
                throw std::invalid_argument(
                    "tensordot: contracted dimensions must match");
            }
        }
    ...
    template <typename T, typename U>
        requires(!np::detail::is_complex_v<T> && !np::detail::is_complex_v<U>)
    auto tensordot(const Ndarray<T>& a, const Ndarray<U>& b, int axes = 2)
        -> Ndarray<std::common_type_t<T, U>> {
        if (axes < 0) {
        ...
        if (ua > na || ua > nb) {
            throw std::invalid_argument(
                "tensordot: axes exceeds the array rank");
        }
        std::vector<int> aa;
        std::vector<int> ba;
    ...
    template <typename T, typename U>
        requires(!np::detail::is_complex_v<T> && !np::detail::is_complex_v<U>)
    auto tensordot(const Ndarray<T>& a, const Ndarray<U>& b, int a_axis,
                   int b_axis) -> Ndarray<std::common_type_t<T, U>> {
        return tensordot(a, b, std::vector<int>{a_axis},
                         std::vector<int>{b_axis});
    }
    ...
    template <typename T, typename U>
        requires(!np::detail::is_complex_v<T> && !np::detail::is_complex_v<U>)
    auto vecdot(const Ndarray<T>& x1, const Ndarray<U>& x2, int axis = -1)
        -> Ndarray<std::common_type_t<T, U>> {
        using R = std::common_type_t<T, U>;
        ...
        if (n1 == 0 || n2 == 0) {
            throw std::invalid_argument(
                "vecdot operands must be non-scalar");
        }
        ...
        if (len1 != len2) {
            throw std::invalid_argument(
                "vecdot: the contracted axes must match in size");
        }
        std::vector<int> r1;
        std::vector<int> r2;
    ...
    template <typename T, typename U>
        requires(!np::detail::is_complex_v<T> && !np::detail::is_complex_v<U>)
    auto cross(const Ndarray<T>& x1, const Ndarray<U>& x2, int axis = -1)
        -> Ndarray<std::common_type_t<T, U>> {
        using R = std::common_type_t<T, U>;
        ...
        if (x1.shape[a1] != 3 || x2.shape[a2] != 3) {
            throw std::invalid_argument(
                "cross: the vectors must have 3 elements");
        }
        std::vector<int> s1;
        std::vector<int> s2;
        ...
        while (!odo.done()) {
            ...
            for (std::size_t i = 0; i < s1.size(); ++i) {
                const std::size_t d = i < static_cast<std::size_t>(a1)
                                          ? i
                                          : i + 1;
                i1[d] = oi[out_of(off1 + i)];
            }
            for (std::size_t i = 0; i < s2.size(); ++i) {
                const std::size_t d = i < static_cast<std::size_t>(a2)
                                          ? i
                                          : i + 1;
            ...
            for (int e = 0; e < 3; ++e) {
                out_idx[static_cast<std::size_t>(vpos)] =
