...
template <typename R>
std::vector<R> take_cols(const std::vector<R> &data, std::size_t rows,
...
template <typename R>
void svd_jacobi(const std::vector<R> &a, std::size_t p, std::size_t q,
                bool full, bool want_uv, std::vector<R> &u, std::vector<R> &s,
                std::vector<R> &v) {
  ...
  std::sort(order.begin(), order.end(), [&norms](std::size_t x, std::size_t y) {
...
// numpy's raw mode.
template <typename R>
void householder_qr(const std::vector<R> &a, std::size_t m, std::size_t n,
                    std::vector<R> &q, std::vector<R> &r, std::vector<R> &h,
                    std::vector<R> &tau) {
...
void francis_schur(std::vector<R> &h, std::size_t n, std::vector<R> &q) {
  ...
  while (p > 0) {
    ...
    while (l > 0) {
      const R tol =
          eps * (std::abs(h[(l - 1) * n + l - 1]) + std::abs(h[l * n + l]));
    ...
    if (nrm == R{0}) {
      ...
      for (std::ptrdiff_t c = l; c <= p; ++c) {
        R dot =
            h[l * n + c] + vy * h[(l + 1) * n + c] + vz * h[(l + 2) * n + c];
        h[l * n + c] -= tau * dot;
...
template <typename R>
std::vector<std::complex<R>> schur_eigenvalues(const std::vector<R> &h,
                                               std::size_t n) {
...
schur_eigenvectors(const std::vector<R> &h, const std::vector<R> &q,
                   std::size_t n, const std::vector<std::complex<R>> &w) {
  ...
  for (std::size_t j = 0; j < n; ++j) {
    ...
    for (std::ptrdiff_t ii = static_cast<std::ptrdiff_t>(n) - 1; ii >= 0;
         --ii) {
      ...
      if (!pair) {
        ...
        if (std::abs(den) <=
            eps * (std::abs(h[u * n + u]) + std::abs(lambda))) {
          z[u] = C{0};
          continue;
        }
        z[u] = -rhs / den;
...
template <typename R>
bool lu_factor(std::vector<R> &a, std::size_t n, std::vector<std::size_t> &piv,
               std::size_t &swaps) {
  piv.resize(n);
  swaps = 0;
...
auto svd(const Ndarray<T> &a, bool full_matrices = true, bool compute_uv = true)
    -> SVDResult<real_t<T>> {
  using R = real_t<T>;
  if (a.ndim() != 2) {
  ...
  if (m >= n) {
    ...
    detail::svd_jacobi(dense_t, n, m, full_matrices, compute_uv, u_vec, s_vec,
                       v_vec);
 ...
 * @complexity O(M * N * min(M, N) * sweeps).
 */
template <typename T> auto svdvals(const Ndarray<T> &a) -> Ndarray<real_t<T>> {
...
auto solve(const Ndarray<T> &a, const Ndarray<U> &b)
  ...
  for (std::size_t c = 0; c < nrhs; ++c) {
    ...
    for (std::size_t i = 0; i < m; ++i) {
      rhs[i] =
          b.ndim() == 1 ? static_cast<R>(b.at(i)) : static_cast<R>(b.at(i, c));
    }
...
template <typename T>
  requires(!np::detail::is_complex_v<T>)
auto cholesky(const Ndarray<T> &a, bool upper = false) -> Ndarray<real_t<T>> {
  using R = real_t<T>;
  if (a.ndim() != 2) {
...
template <typename T>
  requires(!np::detail::is_complex_v<T>)
auto norm(const Ndarray<T> &x, NormOrd ord = NormOrd::None) -> real_t<T> {
  using R = real_t<T>;
...
template <typename T>
  requires(!np::detail::is_complex_v<T>)
auto matrix_norm(const Ndarray<T> &x, NormOrd ord = NormOrd::Fro) -> real_t<T> {
  using R = real_t<T>;
  if (x.ndim() != 2) {
...
auto vector_norm(const Ndarray<T> &x, const std::vector<int> &axis = {},
  ...
  while (!odo.done()) {
    ...
    while (!rdone) {
      ...
      const R v =
          static_cast<R>(std::abs(static_cast<R>(x.data()[x._flat(idx)])));
      if (is_inf) {
        if (first) {
          best = v;
        } else {
...
template <typename T>
  requires(!np::detail::is_complex_v<T>)
auto pinv(const Ndarray<T> &a, double rcond = 1e-15) -> Ndarray<real_t<T>> {
  using R = real_t<T>;
  if (a.ndim() != 2) {
...
auto eigh(const Ndarray<T> &a) -> EighResult<real_t<T>> {
  ...
  std::sort(order.begin(), order.end(), [&e](std::size_t x, std::size_t y) {
...
template <typename T> auto matrix_transpose(const Ndarray<T> &x) -> Ndarray<T> {
  const std::size_t nd = x.ndim();
  if (nd < 2) {
    throw std::invalid_argument(
...
auto tensorsolve(const Ndarray<T> &a, const Ndarray<U> &b,
  ...
  if (axes.empty()) {
    ...
    for (int ax : axes) {
      order.push_back(
          static_cast<int>(ax < 0 ? ax + static_cast<int>(a.ndim()) : ax));
    }
  }
...
template <typename T>
  requires(!np::detail::is_complex_v<T>)
auto matrix_power(const Ndarray<T> &a, long long n) -> Ndarray<real_t<T>> {
  using R = real_t<T>;
  if (a.ndim() != 2) {
...
auto multi_dot(const std::vector<Ndarray<T>> &arrays) -> Ndarray<T> {
  ...
  std::vector<std::vector<std::size_t>> split(n,
...
template <typename T, typename U>
  requires(!np::detail::is_complex_v<T> && !np::detail::is_complex_v<U>)
auto tensordot(const Ndarray<T> &a, const Ndarray<U> &b, int a_axis, int b_axis)
    -> Ndarray<std::common_type_t<T, U>> {
...
template <typename T> auto transpose(const Ndarray<T> &a) -> Ndarray<T> {
  return a.transpose();
}

// Trace of a 2D array.
// Reference: numpy-reference/reference/generated/numpy.trace.html
template <typename T> auto trace(const Ndarray<T> &a) -> T { return a.trace(); }
