 ...
 */
inline void radix2_apply(std::vector<Cplx>& a, std::size_t n, const std::vector<Cplx>& tbl,
                         bool inverse) {
...
public:
    ...
    [[nodiscard]] const BluesteinPlan& bluestein_plan(std::size_t n, bool inverse) const {
...
template <typename T> [[nodiscard]] ndarray<Cplx> to_complex(const ndarray<T>& x) {
...
template <typename T> [[nodiscard]] ndarray<Cplx> conjugate_copy(const ndarray<T>& x) {
    ndarray<Cplx> out(x.shape);
    for (std::size_t i = 0; i < x._numel(); ++i) {
 ...
 */
template <typename T>
inline void transform_lines(const ndarray<T>& src, int axis, std::size_t n, ndarray<Cplx>& dst,
                            bool inverse, double scale, const TwiddleCache& cache) {
 ...
 */
template <typename T>
inline void rfft_lines(const ndarray<T>& src, int axis, std::size_t n, ndarray<Cplx>& dst,
 ...
 */
inline void irfft_lines(const ndarray<Cplx>& src, int axis, std::size_t n, ndarray<double>& dst,
                        double scale, const TwiddleCache& cache) {
