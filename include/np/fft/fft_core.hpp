 ...
 */
enum class Norm : std::uint8_t { Backward, Ortho, Forward };
 ...
 */
inline double scale_factor(const Norm norm, std::size_t len,
...
[[nodiscard]] inline int normalize_axis(int axis, std::size_t nd) {
  if (axis < 0) {
    axis += static_cast<int>(nd);
  }
  if (axis < 0 || axis >= static_cast<int>(nd)) {
    throw np::AxisError("axis " + std::to_string(axis) +
 ...
 */
inline void radix2_apply(std::vector<Cplx> &a, std::size_t n,
                         const std::vector<Cplx> &tbl, bool inverse) {
...
public:
  ...
  [[nodiscard]] const std::vector<Cplx> &radix_table(std::size_t n) const {
    auto it = fwd_.find(n);
    if (it == fwd_.end()) {
  ...
  [[nodiscard]] const BluesteinPlan &bluestein_plan(std::size_t n,
                                                    bool inverse) const {
    ...
    if (it == tbl.end()) {
      ...
      for (std::size_t j = 0; j < n; ++j) {
        const double angle = s * std::numbers::pi_v<double> *
                             static_cast<double>(j * j) * inv_n;
...
[[nodiscard]] inline const TwiddleCache &twiddle_cache() {
  static const TwiddleCache cache;
  return cache;
}

/** @brief In-place radix-2 FFT (n = a.size() must be a power of two). */
inline void radix2(std::vector<Cplx> &a, bool inverse, double scale,
 ...
 */
inline void bluestein(std::vector<Cplx> &x, bool inverse, double scale,
                      const TwiddleCache &cache) {
...
inline void transform(std::vector<Cplx> &a, bool inverse, double scale,
                      const TwiddleCache &cache) {
  const std::size_t n = a.size();
  if (n <= 1) {
    if (scale != 1.0) {
...
inline std::vector<int> with_axis_len(const std::vector<int> &shape, int axis,
                                      std::size_t len) {
...
inline void transform_lines(const ndarray<T> &src, int axis, std::size_t n,
  ...
  while (!od.done()) {
    ...
    const std::size_t base =
        np::detail::flat_index(full, src.strides, src.offset);
    ...
    const std::size_t db =
        np::detail::flat_index(full, dst.strides, dst.offset);
    for (std::size_t k = 0; k < n; ++k) {
...
inline void irfft_lines(const ndarray<Cplx> &src, int axis, std::size_t n,
  ...
  while (!od.done()) {
    ...
    const std::size_t db =
        np::detail::flat_index(full_idx, dst.strides, dst.offset);
    for (std::size_t k = 0; k < n; ++k) {
