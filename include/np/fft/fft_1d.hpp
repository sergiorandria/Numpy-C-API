...
namespace np::fft
{
  ...
  namespace detail
  {
    ...
    deduced_len(bool has_n, std::size_t n, std::size_t axis_len)
    {
  ...
  NP_API NP_NODISCARD inline auto ifft(const std::vector<Cplx>& x) -> std::vector<Cplx>
  {
    std::vector<Cplx> out(x);
    detail::transform(
        out, true, 1.0 / static_cast<double>(out.size()), detail::twiddle_cache());
    return out;
  }
      ...
      Norm norm = Norm::Backward) -> ndarray<Cplx>
  {
    ...
    detail::rfft_lines(
        x, ax, len, out, detail::scale_factor(norm, len, false), detail::twiddle_cache());
    return out;
  }
      ...
      Norm norm = Norm::Backward) -> ndarray<double>
  {
    ...
    detail::irfft_lines(
        cx, ax, len, out, detail::scale_factor(norm, len, true), detail::twiddle_cache());
    return out;
  }
