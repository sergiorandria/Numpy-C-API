...
namespace np::fft
{
  ...
  namespace detail
  {
    ...
    template <typename T>
    NP_NODISCARD ndarray<T> shift_roll(
        const ndarray<T>& arr, const std::optional<std::vector<int>>& axes, int sign)
    {
      ndarray<T> out = arr;
      if (axes)
      {
  ...
  NP_NODISCARD auto
  fftshift(const ndarray<T>& x, std::optional<std::vector<int>> axes = std::nullopt)
      -> ndarray<T>
  {
  ...
  NP_API template <typename T>
  NP_NODISCARD auto
  ifftshift(const ndarray<T>& x, std::optional<std::vector<int>> axes = std::nullopt)
      -> ndarray<T>
  {
    return detail::shift_roll(x, axes, -1);
  }
