...
namespace np::fft
{
      ...
      Norm norm = Norm::Backward) -> ndarray<Cplx>
  {
    ...
    for (std::size_t i = 0; i < plan.axes.size(); ++i)
    {
      ...
      detail::transform_lines(
          cur, a, len, nxt, false, detail::scale_factor(norm, len, false), cache);
      cur = std::move(nxt);
    }
    return cur;
  }
      ...
      Norm norm = Norm::Backward) -> ndarray<Cplx>
  {
    ...
    for (std::size_t i = 0; i < plan.axes.size(); ++i)
    {
      ...
      ndarray<Cplx> nxt(detail::with_axis_len(cur.shape, a, len));
      detail::transform_lines(
          cur, a, len, nxt, true, detail::scale_factor(norm, len, true), cache);
      cur = std::move(nxt);
    }
    return cur;
  }
      ...
      Norm norm = Norm::Backward) -> ndarray<Cplx>
  {
    ...
    detail::rfft_lines(
        x, last_axis, last_len, cur, detail::scale_factor(norm, last_len, false), cache);

    // Complex transforms on the remaining axes, in reverse order.
    for (std::ptrdiff_t j = static_cast<std::ptrdiff_t>(plan.axes.size()) - 2; j >= 0;
         --j)
    {
      ...
      Norm norm = Norm::Backward) -> ndarray<double>
  {
    ...
    detail::irfft_lines(
        cur, last_axis, last_len, out, detail::scale_factor(norm, last_len, true), cache);
    return out;
  }
