...
namespace np
{
  ...
  {
    static_assert(
        std::conjunction_v<std::bool_constant<(Extents > 0)>...>,
  ...
  public:
    ...
    constexpr explicit ndarrayf(std::initializer_list<std::initializer_list<U>> rows)
        : m_data()
    {
    ...
    constexpr auto sum() const
    {
      ...
      else
      {
        ...
        return axis_reduce<V>(
            Axis,
            tag{},
            [](V acc, const V& v) { return acc + v; },
            traits::zero(),
            [](V v) { return traits::make(v); },
    ...
    {
      using tag =
          typename detail::expr::reverse<detail::expr::shape_tag<Extents...>>::type;
    ...
    {
      using tag =
          typename detail::expr::squeeze_tag<detail::expr::shape_tag<Extents...>>::type;
    ...
    {
      static_assert(
          Axis >= 0 && Axis < static_cast<int>(rank), "np: squeeze axis out of range");
      static_assert(
          static_shape[Axis] == 1,
    ...
    constexpr auto expand_dims() const
    {
      ...
      using tag = typename detail::expr::
          insert<1, Axis, 0, detail::expr::shape_tag<Extents...>>::type;
  ...
  private:
    ...
    constexpr auto axis_reduce(
        int axis,
        detail::expr::shape_tag<E...>,
        Op&& op,
        R init,
        Finish&& finish,
    ...
    template <int... E>
    constexpr ndarrayf<T, E...> transpose_impl(detail::expr::shape_tag<E...>) const
    {
    ...
    constexpr ndarrayf<T, E...> squeeze_axis_impl(detail::expr::shape_tag<E...>) const
    {
    ...
    template <int Axis, int... E>
    constexpr ndarrayf<T, E...> expand_dims_impl(detail::expr::shape_tag<E...>) const
    {
      constexpr std::array<int, rank + 1> rshape{E...};
      ndarrayf<T, E...> out{};
      for (std::size_t i = 0; i < size_v; ++i)
  ...
  namespace detail::fixed
  {
    ...
    template <typename S>
    using wrap_t = std::
        conditional_t<detail::expr::fixed_source<S>, S, detail::expr::scalar_expr<S>>;

    /** @brief True when the two operands (arrays or scalars) broadcast. */
    template <typename L, typename R>
    inline constexpr bool binop_ok = detail::expr::broadcast_valid<wrap_t<L>, wrap_t<R>>;
...
#undef NP_FIXED_UNARY_FN
  ...
  namespace detail::fixed
  {
    ...
    constexpr ndarrayf<R, E...>
    concat_impl(detail::expr::shape_tag<E...>, const As&... as)
    {
  ...
  {
    static_assert(
        detail::fixed::concat_ok<A0, Rest...>::valid,
  ...
  {
    static_assert(
        detail::fixed::stack_ok<A0, Rest...>::valid,
    ...
    using tag0 = detail::expr::shape_tag_t<A0>;
    using rtag =
        typename detail::expr::insert<static_cast<int>(count), Axis, 0, tag0>::type;
    using R = std::common_type_t<typename A0::value_type, typename Rest::value_type...>;
