...
namespace np::linalg {
    ...
    namespace detail::fixed {
        ...
        template <typename R, int N>
        constexpr ndarray<R, N, N> lu_invert(
            const ndarray<R, N, N>& lu,
            const std::array<int, static_cast<std::size_t>(N)>& piv) {
            ndarray<R, N, N> out{};
            for (int c = 0; c < N; ++c) {
            ...
            for (int j = 0; j < C; ++j) {
                ...
                for (int m = 0; m < P; ++m) {
                    ...
                    for (int t = 0; t < j; ++t) {
                        R dot{};
                        for (int i = 0; i < P; ++i) {
                            dot += cand[static_cast<std::size_t>(i)] *
                                   data(i, t);
                        }
                        for (int i = 0; i < P; ++i) {
                    ...
                    R cn{};
                    for (int i = 0; i < P; ++i) {
                        cn += cand[static_cast<std::size_t>(i)] *
                              cand[static_cast<std::size_t>(i)];
                    }
                    ...
                    if (cn > R{0.5}) {
                        for (int i = 0; i < P; ++i) {
                            data(i, j) =
                                cand[static_cast<std::size_t>(i)] / cn;
                        }
                        break;
                    }
                }
        ...
        template <typename R, int P, int Q, bool Full>
        constexpr JacobiSvdResult<R, P, Q, Full> jacobi_svd(
            const ndarray<R, P, Q>& a) {
            static_assert(P >= Q,
                ...
                for (int pc = 0; pc < Q; ++pc) {
                    for (int qc = pc + 1; qc < Q; ++qc) {
                        ...
                        if (np::detail::math::abs(gamma) <=
                            eps * np::detail::math::sqrt(
                                      static_cast<double>(alpha * beta))) {
                            continue;
                        }
                        ...
                        const R t =
                            ...
                            (np::detail::math::abs(zeta) +
                             np::detail::math::sqrt(
                                 static_cast<double>(R{1} + zeta * zeta)));
                        const R c = R{1} / np::detail::math::sqrt(
            ...
            for (int j = 0; j < Q; ++j) {
                ...
                norms[static_cast<std::size_t>(j)] =
                    static_cast<R>(np::detail::math::sqrt(
                        static_cast<double>(nrm)));
            }
        ...
        template <typename R, int M, int N>
        constexpr HouseholderQrResult<R, M, N> householder_qr(
            const ndarray<R, M, N>& a) {
            ...
            for (int j = 0; j < K; ++j) {
                ...
                const R nrm = static_cast<R>(np::detail::math::sqrt(
                    static_cast<double>(xj * xj + nrm2)));
                if (nrm == R{0}) {
                    continue;
                }
        ...
        template <typename R, int N>
        constexpr JacobiEighResult<R, N> jacobi_eigh(
            const ndarray<R, N, N>& a) {
            ...
            for (int sweep = 0; sweep < 60; ++sweep) {
                ...
                const R t =
                    ...
                    (np::detail::math::abs(theta) +
                     np::detail::math::sqrt(
                         static_cast<double>(R{1} + theta * theta)));
                const R c = R{1} / np::detail::math::sqrt(
                                          static_cast<double>(R{1} + t * t));
                const R s = c * t;
                for (int k = 0; k < N; ++k) {
                    if (k == pp || k == qq) {
                        continue;
            ...
            for (int j = 0; j < N; ++j) {
                ...
                sorted.w[static_cast<std::size_t>(j)] =
                    out.w[static_cast<std::size_t>(src)];
                for (int i = 0; i < N; ++i) {
                    vp(i, j) = v(i, src);
                }
            }
    ...
    namespace fixed {
        ...
        template <typename R, int URows, int UCols, int SRows, int VRows,
                  int VCols>
        struct SVDResult {
    ...
    template <typename T, int N>
    constexpr auto trace(const ndarray<T, N, N>& a)
    ...
    constexpr auto det(const ndarray<T, N, N>& a)
        -> typename np::detail::fixed::float_t<T> {
        using R = typename np::detail::fixed::float_t<T>;
        ...
        for (int i = 0; i < N; ++i) {
            ...
            logabs += static_cast<R>(np::detail::math::log(
                static_cast<double>(np::detail::math::abs(u))));
        }
        return sign * static_cast<R>(np::detail::math::exp(
                                         static_cast<double>(logabs)));
    }
    ...
    template <typename T, int N>
        requires(!np::detail::is_complex_v<T>)
    constexpr auto slogdet(const ndarray<T, N, N>& a) -> SlogdetResult<
        typename np::detail::fixed::float_t<T>> {
        using R = typename np::detail::fixed::float_t<T>;
        const auto d = detail::fixed::lu_factor(a);
        if (d.singular) {
    ...
    template <typename T, int N>
        requires(!np::detail::is_complex_v<T>)
    constexpr auto inv(const ndarray<T, N, N>& a)
    ...
    constexpr auto solve(const ndarray<T, N, N>& a, const ndarray<U, N>& b)
    ...
    template <typename T, typename U, int N, int M>
        requires(!np::detail::is_complex_v<T> && !np::detail::is_complex_v<U>)
    constexpr auto solve(const ndarray<T, N, N>& a, const ndarray<U, N, M>& b)
        -> ndarray<std::common_type_t<typename np::detail::fixed::float_t<T>,
                                      typename np::detail::fixed::float_t<U>>,
                   N, M> {
        using R = std::common_type_t<typename np::detail::fixed::float_t<T>,
    ...
    template <typename T, int N>
        requires(!np::detail::is_complex_v<T>)
    constexpr auto cholesky(const ndarray<T, N, N>& a, bool upper = false)
        -> ndarray<typename np::detail::fixed::float_t<T>, N, N> {
        using R = typename np::detail::fixed::float_t<T>;
        ndarray<R, N, N> l{};
        for (int i = 0; i < N; ++i) {
            for (int j = 0; j <= i; ++j) {
                ...
                if (i == j) {
                    ...
                    l(i, j) = static_cast<R>(np::detail::math::sqrt(
    ...
    template <typename T, int N>
        requires(!np::detail::is_complex_v<T>)
    constexpr auto matrix_power(const ndarray<T, N, N>& a, long long n)
        -> ndarray<typename np::detail::fixed::float_t<T>, N, N> {
        using R = typename np::detail::fixed::float_t<T>;
    ...
    template <typename T, int M, int N>
        requires(!np::detail::is_complex_v<T>)
    constexpr auto svdvals(const ndarray<T, M, N>& a)
    ...
    template <bool Full = true, typename T, int M, int N>
        requires(!np::detail::is_complex_v<T>)
    constexpr auto svd(const ndarray<T, M, N>& a) {
        using R = typename np::detail::fixed::float_t<T>;
        constexpr int K = M < N ? M : N;
        ndarray<R, M, N> dense{};
        for (int i = 0; i < M; ++i) {
    ...
    template <bool Reduced = true, typename T, int M, int N>
        requires(!np::detail::is_complex_v<T>)
    constexpr auto qr(const ndarray<T, M, N>& a) {
        using R = typename np::detail::fixed::float_t<T>;
        constexpr int K = M < N ? M : N;
        ndarray<R, M, N> dense{};
        for (int i = 0; i < M; ++i) {
    ...
    template <typename T, int M>
        requires(!np::detail::is_complex_v<T>)
    constexpr auto norm(const ndarray<T, M>& x, NormOrd ord = NormOrd::None)
        -> typename np::detail::fixed::float_t<T> {
        using R = typename np::detail::fixed::float_t<T>;
        ...
        switch (ord) {
            ...
            case NormOrd::Two: {
                ...
                return static_cast<R>(
                    np::detail::math::sqrt(static_cast<double>(acc)));
            }
            case NormOrd::One: {
                R acc{};
                for (int i = 0; i < M; ++i) {
                    acc += static_cast<R>(np::detail::math::abs(
                        static_cast<R>(x[i])));
                }
                return acc;
            }
            case NormOrd::Inf: {
                R best{};
                for (int i = 0; i < M; ++i) {
                    const R v = static_cast<R>(np::detail::math::abs(
                        static_cast<R>(x[i])));
                    best = best < v ? v : best;
                }
                return best;
            ...
            case NormOrd::NegOne: {
                R acc{};
                for (int i = 0; i < M; ++i) {
                    acc += R{1} / static_cast<R>(np::detail::math::abs(
                                             static_cast<R>(x[i])));
                }
    ...
    template <typename T, int M, int N>
        requires(!np::detail::is_complex_v<T>)
    constexpr auto norm(const ndarray<T, M, N>& x, NormOrd ord = NormOrd::None)
        -> typename np::detail::fixed::float_t<T> {
        using R = typename np::detail::fixed::float_t<T>;
        constexpr int K = M < N ? M : N;
        switch (ord) {
            ...
            case NormOrd::NegOne: {
                ...
                for (int j = 0; j < N; ++j) {
                    ...
                    best = ord == NormOrd::One ? (best < acc ? acc : best)
                                               : (acc < best ? acc : best);
                }
                return best == std::numeric_limits<R>::infinity() ? R{0}
                                                                  : best;
            ...
            case NormOrd::NegInf: {
                ...
                for (int i = 0; i < M; ++i) {
                    ...
                    best = ord == NormOrd::Inf ? (best < acc ? acc : best)
                                               : (acc < best ? acc : best);
                }
                return best == std::numeric_limits<R>::infinity() ? R{0}
                                                                  : best;
    ...
    template <typename T, int M, int N>
        requires(!np::detail::is_complex_v<T>)
    constexpr auto matrix_rank(const ndarray<T, M, N>& a,
                               double tol = -1.0) -> int {
        const auto s = svdvals(a);
        const double t = tol < 0.0
    ...
    template <typename T, int N>
        requires(!np::detail::is_complex_v<T>)
    constexpr auto matrix_rank(const ndarray<T, N>& a) -> int {
        for (int i = 0; i < N; ++i) {
            if (a[i] != T{0}) {
                return 1;
            }
    ...
    template <typename T, int M, int N>
        requires(!np::detail::is_complex_v<T>)
    constexpr auto pinv(const ndarray<T, M, N>& a, double rcond = 1e-15)
        -> ndarray<typename np::detail::fixed::float_t<T>, N, M> {
        using R = typename np::detail::fixed::float_t<T>;
        constexpr int K = M < N ? M : N;
        const auto r = svd<false>(a);
        const double cutoff =
            static_cast<double>(r.s[0]) * static_cast<double>(M < N ? N : M) *
            rcond;
        ndarray<R, N, M> out{};
        for (int i = 0; i < N; ++i) {
            for (int j = 0; j < M; ++j) {
                R acc{};
                for (int k = 0; k < K; ++k) {
                    const R sp = static_cast<double>(r.s[k]) > cutoff
                                     ? R{1} / r.s[k]
                                     : R{0};
    ...
    template <typename T, int N>
        requires(!np::detail::is_complex_v<T>)
    constexpr auto cond(const ndarray<T, N, N>& a)
        -> typename np::detail::fixed::float_t<T> {
        using R = typename np::detail::fixed::float_t<T>;
        const auto s = svdvals(a);
        return s[N - 1] == R{0}
    ...
    template <typename T, int N>
        requires(!np::detail::is_complex_v<T>)
    constexpr auto cond(const ndarray<T, N, N>& a, NormOrd ord)
        -> typename np::detail::fixed::float_t<T> {
        using R = typename np::detail::fixed::float_t<T>;
    ...
    template <typename T, int N>
        requires(!np::detail::is_complex_v<T>)
    constexpr auto eigh(const ndarray<T, N, N>& a)
    ...
    template <typename T, int N>
        requires(!np::detail::is_complex_v<T>)
    constexpr auto eigvalsh(const ndarray<T, N, N>& a)
    ...
    template <typename T, typename U>
        requires(!np::detail::is_complex_v<T> && !np::detail::is_complex_v<U>)
    constexpr auto cross(const ndarray<T, 3>& a, const ndarray<U, 3>& b)
    ...
    template <typename T, typename U, int M>
        requires(!np::detail::is_complex_v<T> && !np::detail::is_complex_v<U>)
    constexpr auto cross(const ndarray<T, M, 3>& a, const ndarray<U, M, 3>& b)
    ...
    template <typename T, typename U, int K, int L>
        requires(!np::detail::is_complex_v<T> && !np::detail::is_complex_v<U>)
    constexpr auto outer(const ndarray<T, K>& a, const ndarray<U, L>& b)
    ...
    template <typename T, typename U, int N>
        requires(!np::detail::is_complex_v<T> && !np::detail::is_complex_v<U>)
    constexpr auto inner(const ndarray<T, N>& a, const ndarray<U, N>& b) {
        return dot(a, b);
    }
    ...
    template <typename T, typename U, int N, int K, int M>
        requires(!np::detail::is_complex_v<T> && !np::detail::is_complex_v<U>)
    constexpr auto inner(const ndarray<T, N, K>& a, const ndarray<U, M, K>& b)
    ...
    template <typename T, typename U, int M, int N>
        requires(!np::detail::is_complex_v<T> && !np::detail::is_complex_v<U>)
    constexpr auto lstsq(const ndarray<T, M, N>& a, const ndarray<U, M>& b)
        -> fixed::LstsqResult<std::common_type_t<
                                  typename np::detail::fixed::float_t<T>,
                                  typename np::detail::fixed::float_t<U>>,
                              N, (M < N ? M : N)> {
        using R = std::common_type_t<typename np::detail::fixed::float_t<T>,
        ...
        const auto s = svdvals(a);
        const double cutoff =
            static_cast<double>(s[0]) * static_cast<double>(M < N ? N : M) *
            std::numeric_limits<double>::epsilon();
        const auto p = pinv(a);
        ndarray<R, N> x{};
        for (int i = 0; i < N; ++i) {
