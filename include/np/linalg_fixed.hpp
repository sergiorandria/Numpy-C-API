  ...
  for (int sweep = 0; sweep < max_sweeps && !converged; ++sweep) {
    ...
    for (int pc = 0; pc < Q; ++pc) {
      for (int qc = pc + 1; qc < Q; ++qc) {
        ...
        const R c =
            R{1} / np::detail::math::sqrt(static_cast<double>(R{1} + t * t));
        const R sn = c * t;
        for (int i = 0; i < P; ++i) {
          const R x = b(i, pc);
          const R y = b(i, qc);
...
householder_qr(const ndarray<R, M, N> &a) {
  ...
  for (int j = 0; j < K; ++j) {
    ...
    const R nrm = static_cast<R>(
        np::detail::math::sqrt(static_cast<double>(xj * xj + nrm2)));
    if (nrm == R{0}) {
      continue;
    }
...
constexpr auto det(const ndarray<T, N, N> &a) ->
  ...
  return sign *
         static_cast<R>(np::detail::math::exp(static_cast<double>(logabs)));
}
...
template <typename T, int N>
  requires(!np::detail::is_complex_v<T>)
constexpr auto slogdet(const ndarray<T, N, N> &a)
  ...
  for (int i = 0; i < N; ++i) {
    for (int j = 0; j <= i; ++j) {
      ...
      if (i == j) {
        ...
        l(i, j) =
            static_cast<R>(np::detail::math::sqrt(static_cast<double>(acc)));
      } else {
        l(i, j) = acc / l(j, j);
      }
    }
...
template <typename T, int M, int N>
  requires(!np::detail::is_complex_v<T>)
constexpr auto matrix_rank(const ndarray<T, M, N> &a, double tol = -1.0)
    -> int {
...
constexpr auto pinv(const ndarray<T, M, N> &a, double rcond = 1e-15)
  ...
  for (int i = 0; i < N; ++i) {
    for (int j = 0; j < M; ++j) {
      ...
      for (int k = 0; k < K; ++k) {
        const R sp =
            static_cast<double>(r.s[k]) > cutoff ? R{1} / r.s[k] : R{0};
...
constexpr auto cond(const ndarray<T, N, N> &a) ->
  ...
  using R = typename np::detail::fixed::float_t<T>;
  const auto s = svdvals(a);
  return s[N - 1] == R{0} ? std::numeric_limits<R>::infinity()
                          : s[0] / s[N - 1];
}
...
constexpr auto lstsq(const ndarray<T, M, N> &a, const ndarray<U, M> &b)
  ...
  constexpr int K = M < N ? M : N;
  const auto s = svdvals(a);
  const double cutoff = static_cast<double>(s[0]) *
                        static_cast<double>(M < N ? N : M) *
                        std::numeric_limits<double>::epsilon();
  const auto p = pinv(a);
  ndarray<R, N> x{};
