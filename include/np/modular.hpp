/**
 * @file modular.hpp
 * @brief Modular forms, q-expansions, Eisenstein series, Dedekind eta, j-invariant, Hecke operators.
 *
...
namespace np::modular
{
  ...
  NP_NODISCARD inline bigint sigma(int k, int n)
  {
    ...
    for (int d = 1; d * d <= n; ++d)
    {
      if (n % d == 0)
      {
        // d^k
        bigint p = 1;
        for (int i = 0; i < k; ++i) p *= bigint(d);
        s += p;
        int other = n / d;
        if (other != d)
        {
          bigint q = 1;
          for (int i = 0; i < k; ++i) q *= bigint(other);
          s += q;
        }
      }
    }
  ...
  NP_NODISCARD inline std::pair<bigint, bigint> bernoulli(int k)
  {
    ...
    switch (k)
    {
      case 0: return {bigint(1), bigint(1)};
      case 2: return {bigint(1), bigint(6)};
      case 4: return {bigint(-1), bigint(30)};
      case 6: return {bigint(1), bigint(42)};
      case 8: return {bigint(-1), bigint(30)};
      case 10: return {bigint(5), bigint(66)};
      case 12: return {bigint(-691), bigint(2730)};
      case 14: return {bigint(7), bigint(6)};
      default: throw std::invalid_argument("bernoulli: only k=0,2,4,6,8,10,12,14 supported");
    }
  }
  ...
  {
    if (k < 4 || k % 2 != 0) throw std::invalid_argument("eisenstein_series: need even k>=4");
    if (N <= 0) throw std::invalid_argument("eisenstein_series: N>0");
  ...
  NP_NODISCARD inline ndarray<bigint> eisenstein_E4(int N) { return eisenstein_series(4, N); }
  NP_NODISCARD inline ndarray<bigint> eisenstein_E6(int N) { return eisenstein_series(6, N); }
  ...
  {
    if (terms <= 0) throw std::invalid_argument("dedekind_eta: terms>0");
    ...
    std::complex<double> phase = std::exp(std::complex<double>(0, pi * tau.real() / 6.0) * std::complex<double>(0, pi * tau.imag() / 6.0));
    // Simpler: q^{1/24}
  ...
  /**
   * @brief Modular discriminant `Δ(τ) = (2π)^{12} η(τ)^{24}` via q-expansion `q ∏(1-q^n)^{24}`.
   * Returns `q * ∏_{n=1}^{terms} (1 - q^n)^{24}` (without `(2π)^{12}` factor for algebraic normalization).
   */
  ...
  {
    if (N <= 0) throw std::invalid_argument("ramanujan_tau: N>0");
    ...
    for (int n = 1; n < N; ++n)
    {
      ...
      auto binom = [](int m, int k) -> bigint {
        if (k < 0 || k > m) return bigint(0);
        bigint res = 1;
        for (int i = 1; i <= k; ++i) { res *= bigint(m - k + i); res /= bigint(i); }
        return res;
      };
      for (int i = 0; i < N; ++i)
      {
    ...
    tau.at(0) = bigint(0);
    for (int n = 1; n < N; ++n) tau.at(static_cast<std::size_t>(n)) = prod[n - 1];
    return tau;
  }
  ...
  {
    if (N <= 0) throw std::invalid_argument("j_invariant_series: N>0");
    ...
    if (N > 0) out.at(0) = 744;
    if (N > 1) out.at(1) = 196884;
    if (N > 2) out.at(2) = 21493760;
    for (int i = 3; i < N; ++i) out.at(static_cast<std::size_t>(i)) = 0; // placeholder
    return out;
  }
  ...
  {
    if (p <= 1) throw std::invalid_argument("hecke_operator: p must be prime >1");
    if (k < 2) throw std::invalid_argument("hecke_operator: k>=2");
    int N = a.shape[0];
    ndarray<bigint> b(std::vector<int>{N});
    bigint p_pow = 1;
    for (int i = 0; i < k - 1; ++i) p_pow *= bigint(p);
    for (int n = 0; n < N; ++n)
    {
      bigint term1 = 0;
      if (p * n < N) term1 = a.at(static_cast<std::size_t>(p * n));
      bigint term2 = 0;
      if (n % p == 0) term2 = p_pow * a.at(static_cast<std::size_t>(n / p));
  ...
  {
    if (p <= 1) throw std::invalid_argument("hecke_operator: p prime");
    int N = a.shape[0];
