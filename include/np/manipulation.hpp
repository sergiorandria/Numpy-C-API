 ...
 * @brief Array manipulation routines (tile, flip, roll, split, delete, insert, etc.).
 *
...
namespace np {
     ...
     */
    template <typename T>
    auto flip(const Ndarray<T>& arr, std::optional<int> axis = std::nullopt) -> Ndarray<T> {
     ...
     */
    template <typename T>
    auto fliplr(const Ndarray<T>& arr) -> Ndarray<T> {
        if (arr.ndim() < 2) {
     ...
     */
    template <typename T>
    auto flipud(const Ndarray<T>& arr) -> Ndarray<T> {
        if (arr.ndim() < 1) {
     ...
     */
    template <typename T>
    auto roll(const Ndarray<T>& arr, int shift, std::optional<int> axis = std::nullopt) -> Ndarray<T> {
        if (!axis.has_value()) {
     ...
     */
    template <typename T>
    auto rot90(const Ndarray<T>& arr, int k = 1, const std::vector<int>& axes = {0, 1}) -> Ndarray<T> {
        if (arr.ndim() < 2) {
     ...
     */
    template <typename T>
    auto diag(const Ndarray<T>& v, int k = 0) -> Ndarray<T> {
        if (v.ndim() == 1) {
     ...
     */
    template <typename T>
    auto vander(const Ndarray<T>& x, int n = -1, bool increasing = false) -> Ndarray<T> {
        if (x.ndim() != 1) {
     ...
     * @tparam T Element type
     * @param arr Input array
     * @param indices_or_sections Indices where to split, or number of equal sections
     * @param axis Axis along which to split
     * @return Vector of sub-arrays
     */
    template <typename T>
    auto split(const Ndarray<T>& arr, const std::vector<int>& indices_or_sections, int axis = 0) -> std::vector<Ndarray<T>> {
        // Normalize axis
        if (axis < 0) {
     ...
     */
    template <typename T>
    auto array_split(const Ndarray<T>& arr, int sections, int axis = 0) -> std::vector<Ndarray<T>> {
        if (axis < 0) {
            axis += static_cast<int>(arr.ndim());
        }
        if (axis < 0 || axis >= static_cast<int>(arr.ndim())) {
     ...
     */
    template <typename T>
    auto hsplit(const Ndarray<T>& arr, const std::vector<int>& indices_or_sections) -> std::vector<Ndarray<T>> {
        if (arr.ndim() == 0) {
     ...
     */
    template <typename T>
    auto vsplit(const Ndarray<T>& arr, const std::vector<int>& indices_or_sections) -> std::vector<Ndarray<T>> {
        if (arr.ndim() < 2) {
     ...
     */
    template <typename T>
    auto dsplit(const Ndarray<T>& arr, const std::vector<int>& indices_or_sections) -> std::vector<Ndarray<T>> {
        if (arr.ndim() < 3) {
     ...
     */
    template <typename T>
    auto delete_arr(const Ndarray<T>& arr, const std::vector<int>& indices, std::optional<int> axis = std::nullopt) -> Ndarray<T> {
        if (indices.empty()) {
            return arr.copy();
        }
     ...
     */
    template <typename T>
    auto insert(const Ndarray<T>& arr, const std::vector<int>& indices, const Ndarray<T>& values, std::optional<int> axis = std::nullopt) -> Ndarray<T> {
        if (indices.empty()) {
            return arr.copy();
        }
     ...
     */
    template <typename T>
    auto append(const Ndarray<T>& arr, const Ndarray<T>& values, std::optional<int> axis = std::nullopt) -> Ndarray<T> {
        if (!axis.has_value()) {
     ...
     */
    template <typename T>
    auto trim_zeros(const Ndarray<T>& arr, const std::string& trim = "fb") -> Ndarray<T> {
        if (arr.ndim() != 1) {
     ...
     */
    template <typename T>
    auto where(const Ndarray<bool>& condition, const Ndarray<T>& x, const Ndarray<T>& y) -> Ndarray<T> {
     ...
     *
     * Reference: numpy-reference/reference/generated/numpy.where.html (single argument form)
     *
     * @param condition Boolean array
